...
- v13.19 (Build 140) — 2026-04-27
    - 兒女生涯記錄編輯頁面：備註下方新增「插入圖片」章節
    - 支援 PhotosPicker 選擇照片、「轉為素描畫」Toggle 開關
    - 素描效果使用 CIColorDodgeBlendMode + GaussianBlur + Grayscale（鉛筆畫風格）
    - 圖片存於 Documents/ChildRecordPhotos/，模型只存檔名
    - ChildRecord 模型新增 photoFileName 欄位（向下相容）
    - 記錄列表有圖片時顯示縮圖預覽

- v13.9 (Build 130) — 2026-04-26
    - 編輯固定支出房貸與車貸採用相同欄位：總貸款金額、開始日期、貸款年限、實際年利率（估算）
    - 房貸的 totalPeriods 由貸款年限自動計算（年×12），同步至連結房地產
    - 開始日期用於房貸 startDate，日期欄位於房貸/車貸下顯示為「開始日期」
    - 編輯既有房貸時自動還原貸款年限與起始日（從連結的 RealEstateMortgageItem 同步）
    - 扣款目標選單改顯示「銀行名稱（NT$ 餘額）」，方便辨識帳戶
    - 餘額計算考慮真實存款 + 連結信用卡支出彙總

- v13.8 (Build 129) — 2026-04-26
    - 新增家庭成員的角色清單在配偶下方新增「爸爸」「媽媽」兩個選項
    - 兒子/女兒之前可選擇父母關係，方便記錄家中長輩

- v13.7 (Build 128) — 2026-04-26
    - 編輯房地產頁面的貸款項目/已支出/變動支出三章節改用 AddExpenseView 統一編輯
    - 點擊既有項目 → 開啟對應類別的 AddExpenseView 編輯
    - 「新增貸款項目」→ 開啟固定支出房貸編輯（含房地產自動連結）
    - 「新增已支出項目」→ 開啟變動支出房屋價金編輯
    - 「新增變動支出」→ 顯示類別選擇 → 對應 AddExpenseView
    - 為新房地產自動 auto-save 到 financeStore，確保 Expense 連結正確
    - AddExpensePreset 新增 realEstateExpenseCategory / linkedRealEstateId / realEstateLinkExisting / mortgageLinkExisting 欄位
    - 項目支援滑動刪除，同步移除連結的 Expense

- v13.6 (Build 127) — 2026-04-26
    - 房地產卡片電梯資料章節右上新增「+」按鈕，可直接新增/編輯保養記錄
    - 房地產卡片水電瓦斯章節右上新增「+」按鈕，可新增/編輯繳費紀錄（類型/日期/金額/收據照片/備註）
    - 新增 UtilityPayment 模型與 UtilityType 列舉，RealEstate 新增 utilityPayments 欄位（向下相容）
    - 新增 ElevatorMaintenanceEditor 與 UtilityPaymentEditor 編輯卡
    - PhotoViewerSheet 支援放大/縮小（最多 5x）、拖曳、雙擊縮放與還原

- v13.5 (Build 126) — 2026-04-26
    - 財富卡片內所有花費項目皆可點擊開啟對應編輯頁
    - 連結支出 → AddExpenseView；連結收入 → AddIncomeView
    - 連結股票 → AddStockView；信用卡彙總條目 → 對應信用卡卡片
    - 信用卡卡片消費趨勢項目改為按鈕，點擊開啟對應 Expense 編輯
    - 新增 handleDepositTap 與 matchingAggregatedCard 工具函式

- v13.4 (Build 125) — 2026-04-26
    - 信用卡卡片「月扣款金額」章節改名為「消費趨勢」
    - 圖表改為依消費日逐筆顯示（每筆一根柱子，同日多筆才合併）
    - 資料量 > 12 筆時自動切換為折線圖（紅色，含漸層 + 節點）
    - 日期軸依資料量自動抽樣顯示，最多 6 個標籤

- v13.3 (Build 124) — 2026-04-26
    - 財富頁面銀行項目下方縮排顯示連結的信用卡（卡名、末四碼、帳單/繳款日）
    - 修正信用卡繳款日計算 bug：繳款日 > 帳單日時（如帳單15日繳款30日），月偏移量改為 0 而非 +1
    - 信用卡卡片「月扣款金額」章節：最近一期顯示加總，下方改為個別消費項目列表（非加總）
    - 信用卡項目列表超過 6 筆顯示「展開全部 / 收起」按鈕
    - 信用卡圖表日期軸依資料量自動抽樣避免擁擠

- v13.2 (Build 123) — 2026-04-26
    - 固定支出貸款章節標題金額改為只計入已開始繳款的項目（date ≤ now），未來日期不計
    - 所有固定支出章節標題金額改以月等效金額顯示（年繳 ÷ 12、季繳 ÷ 3）
    - 固定支出保險章節標題金額依幣別分組顯示（如「NT$50,000 + US$ 1,000」）
    - 固定支出列表各項目金額顯示改依 Expense.currencyCode 判斷，非 NT$ 顯示原始幣別
    - 分類排序也改用已生效項目的金額

- v13.1 (Build 122) — 2026-04-26
    - 財富頁面銀行項目右側改為「開戶日期：YYYY/M/D」+ 下方顯示「存款總額 NT$ X」
    - 銀行存款總額計算包含真實存款 + 連結信用卡支出彙總（即時運算）
    - 財富卡片銀行存款圖表「目前餘額」改名為「目前總額」
    - 存款資料超過 12 筆時自動切換為折線圖（含零線、漸層填色、節點）
    - 日期軸依資料量自動抽樣顯示（最多顯示 6 個日期標籤）避免擠壓
    - 銀行存款列表預設只顯示最近 6 筆，超過時提供「展開全部 / 收起」按鈕

- v13.0 (Build 121) — 2026-04-26
    - 編輯股票頁面：買入日期上方新增「扣款帳戶」選單（銀行/證券分組）
    - 買入時自動寫入帳戶扣款記錄；賣出時依損益自動寫入存入或扣款記錄
    - 證券財富卡片新增交易記錄章節（與銀行同邏輯的柱狀圖+列表）
    - 股票交易條目顯示「買入/虧損」（紫色）或「賣出獲利」（紫色）徽章
    - Stock 模型新增 linkedBankMilestoneId/linkedBankCurrency/linkedSecuritiesMilestoneId 欄位，向下相容
    - BankDeposit 模型新增 linkedStockId 欄位
    - 刪除股票時自動清除連結帳戶的相關記錄
