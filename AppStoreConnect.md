# LifeGood — App Store Connect 上架資訊

> 以下為 App Store Connect 所需的所有文字欄位，分為繁體中文（主要）及英文（國際化）版本。
...
五種時間維度一鍵切換：
  ...
  · 季總結 — 近 8 季支出概覽
  · 年總結 — 近 5 年年度趨勢
互動式長條圖搭配折線圖，支援觸控查看詳細金額。提供總計、平均、最高三大關鍵數據，以及變動與固定支出的比例分析。
...
Switch between 5 time dimensions:
  · Daily — last 30 days
  · Weekly — last 12 weeks
  · Monthly — last 12 months
  · Quarterly — last 8 quarters
  · Yearly — last 5 years
Interactive bar + line charts with touch-to-inspect. Key stats include total, average, and peak spending, plus a variable vs. fixed ratio breakdown.
...
## 4. 宣傳文字 (Promotional Text, 170字內)

### 繁體中文
```
全新記帳體驗！LifeGood 幫您輕鬆追蹤變動與固定支出，五種時間維度的趨勢圖表讓財務一目瞭然。完全免費、離線使用、隱私優先。
```

### English
```
A fresh way to track expenses! LifeGood helps you manage variable & fixed spending with trend charts across 5 time dimensions. Free, offline, privacy-first.
```

---

## 5. 關鍵字 (Keywords, 100字內，逗號分隔)

### 繁體中文
```
記帳,支出,理財,開支,帳本,花費,預算,固定支出,變動支出,圖表,趨勢,財務管理,省錢,日記帳,月記帳
```

### English
```
expense,tracker,budget,finance,spending,money,chart,trend,fixed,variable,bookkeeping,ledger,saving
```
...
### App Privacy Details (App Store Connect 選填)
...
> App Store Connect 隱私標籤選擇：**不收集資料 (Data Not Collected)**
>
> 本 App 所有資料（支出紀錄、分類、金額）皆儲存於使用者裝置的 UserDefaults 中，
> 不會透過網路傳送至任何伺服器，也不包含任何分析或追蹤 SDK。
...
LifeGood（以下稱「本 App」）尊重並保護您的隱私。

1. 資料收集
   本 App 不會收集、傳送或儲存任何個人資料至外部伺服器。所有支出紀錄、分類及設定資料
   均儲存於您裝置的本機儲存空間中。

2. 第三方服務
   本 App 不使用任何第三方分析工具、廣告 SDK 或追蹤技術。

3. 網路存取
   本 App 不需要網路連線即可運作，且不會發起任何網路請求。

4. 資料刪除
   刪除本 App 將同時移除所有儲存於裝置上的資料。

5. 聯絡方式
   如有任何隱私相關問題，請來信：[your-email@example.com]
```
...
| 欄位 | 建議內容 |
|------|---------|
| **支援網址 (Support URL)** | 請替換為您的支援頁面或 GitHub 連結 |
| **行銷網址 (Marketing URL)** | 選填，可用 GitHub Pages 或官網 |
| **版權 (Copyright)** | © 2026 LifeGood. All rights reserved. |
| **授權合約** | 使用 Apple 標準 EULA（預設值即可） |

---

## 10. App 預覽與截圖建議

上架需提供 iPhone 6.7" 及 6.5" 截圖，建議截取以下畫面：
