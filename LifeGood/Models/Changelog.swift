...
enum Changelog {
    static let entries: [ChangelogEntry] = [
        ChangelogEntry(version: "25.398", build: 1151, date: "2026/08/19", notes: [
            "【新增】名片頁右上角多了公司篩選，可以把不想看的公司整批藏起來。沒填公司的名片歸在「未分類」，也能單獨隱藏。設定會記住，離開頁面再回來還在。",
            "【說明】畫面上是勾「要顯示哪幾家」，但實際存的是「被隱藏的那幾家」——這樣之後新增的公司預設一定看得到，不會被舊設定莫名其妙擋掉。",
            "【新增】有隱藏時清單上方會出現橫幅：「顯示 12 / 47 張・已隱藏：某某公司」，點 ✕ 一鍵全部還原。篩選鈕本身也會從灰線框變成橘色實心，一眼看得出現在有篩選在作用。",
            "【說明】這個設定只影響名片清單的顯示，不會刪掉任何名片，也不影響匯出、搜尋以外的其他頁面。",
            "【修正】把所有公司都關掉時，原本會顯示「尚無名片紀錄」，看起來像資料不見了。改成「這些公司目前被隱藏」並告訴你去右上角打開；搜尋不到時的提示也會順帶提醒有篩選在作用。"
        ]),
        ChangelogEntry(version: "25.397", build: 1150, date: "2026/08/19", notes: [
            "【新增】載具編輯多了兩個選填欄位（只有電車／混合動力／電動機車才出現）：「原廠電池容量」與「家充地點關鍵字」。填了才算得出充電損耗、續航與家充佔比；不填不影響任何既有功能。",
            "【新增】載具詳情的充電統計下方多了「充電與里程分析」，點進去是十個區塊的完整分析。",
            "【新增】續航推估：可用容量、滿電續航、每 1% 電量約幾公里，以及「最後一次充到 68%，約還能跑 231 km」。",
            "【新增】電池健康度。⚠️ 這裡刻意是「跟自己比」而不是跟原廠比——充電樁打出去的度數含充電損耗，所以推估容量天生比原廠標稱高，直接除原廠會算出 105～115%，看起來像電池變大了。改成取最早 5 筆與最新 5 筆的中位數相比，顯示「相對初期 −3.2%」。有設家充關鍵字時只用樣本較多的那一組算（家充與快充效率差很多，混在一起的趨勢是假的）。",
            "【新增】充電損耗率＝充入度數 ÷（充電 % × 原廠容量）− 1，家充與外部分開算。AC 家充一般 10～15%；這個數字突然變高通常是充電器或電池出狀況的早期訊號——這才是原廠容量能誠實回答的問題。",
            "【新增】家充佔比（用度數而不是次數算，用次數會低估家充扛下來的比例）、家充與外部電價對照，以及「這些家充度數若都在外面充要多付多少錢」。",
            "【新增】充電地點排行：次數、累計度數、累計花費與每度電價，找得出最貴的那幾站。",
            "【新增】充電習慣：平均起充 %、平均充到 %、充到滿次數、深放電次數，加上起始電量分布長條圖。長期從很低才充或常充到 100% 都會加速衰退——這是這裡唯一真的能延長電池壽命的一組指標，而且只用起訖 % 就算得出來。",
            "【新增】電耗與成本：平均電耗、每公里電費、統計里程、年化里程，加上電耗走勢。每月行駛里程長條圖與每公里成本月趨勢。",
            "【新增】停車熱點：依次數排行並畫在地圖上（圓圈大小＝次數）。沒有經緯度的地點會列在清單但不上圖。",
            "【說明】每一段都有自己的樣本門檻，資料不夠時會直接寫「還差幾筆」而不是畫一條會騙人的線。年化里程要有跨度 30 天以上才推算——拿兩週的資料乘 26 倍是在編故事。"
        ]),
        ChangelogEntry(version: "25.396", build: 1149, date: "2026/08/19", notes: [
            "【新增】從我的行事曆或部屬總覽按「＋」新增部屬任務／會議／報告時，選人的畫面多了搜尋欄，姓名、課別、職稱、廠區都能打。部屬一多，從頭捲到尾找人太花時間。",
            "【新增】選人清單改成依課別分組，每一組標著人數。搜尋與分組是同一個畫面，不打字也比原本一長串好找。",
            "【修正】姓名排序原本是用字串直接比大小，中文姓名比出來的是 Unicode 碼位序，看起來像亂排的。改用中文排序規則。「未分課別」的人固定排在最後，不會夾在中間。",
            "【說明】這兩個頁面的新增走的是同一個選人畫面，所以兩邊一起生效。"
        ]),
        ChangelogEntry(version: "25.395", build: 1148, date: "2026/08/19", notes: [
            "【美化】人才矩陣的統計圖表，選「只顯示某個人」匯出時，那個人現在會被打上聚光燈：他的橫條維持原本的主題色，其他人的條淡化到三成當背景，名字與數值也一併加粗上色。",
            "【說明】只把他那一條變色是不夠的。十幾條同色的橫條並排時，單靠一條的色差在縮圖上根本看不出來——要把其他人壓暗，那一條才會跳出來。",
            "【美化】左側的名字也跟著加粗上色。只有條變色的話，看圖的人得先自己數到第幾條才知道是誰。圖卡標題旁邊也多了一顆寫著他名字的膠囊。",
            "【新增】圖卡下方多一行結論：「王小明：18 小時・第 3 名／共 8 人・高於團隊平均 4.2」。只挑一個人匯出時，「他排第幾」往往才是真正想問的事。名次不把「團隊平均」那條算進去——它是基準不是人；那個人在這張圖沒有資料時會直接講明，不留白。"
        ]),
        ChangelogEntry(version: "25.394", build: 1147, date: "2026/08/19", notes: [
            "【修正】找到公司組織「部門人員」人數會變兩倍的原因了。刪除部屬時，公司組織裡的那個人會被保留下來（他還在公司，只是不再是你的部屬），只解除兩邊的連結；之後同一個人重新被加成部屬時——重新加、從備份還原、或換手機後 iCloud 把部屬資料拉回來——系統找不到「已連結」的人員，就會再建一筆全新的。部屬清單曾經整批重來過的話，整個部門就會變成兩倍。",
            "【修正】新增部屬時，現在會先找有沒有同名、同部門、而且還沒連結任何部屬的既有人員可以認領，認領了就不再新建，重複不會再繼續累積。",
            "【新增】已經產生的重複要自己清。部門有重複人員時，「部門人員」下方會出現橘色提示「偵測到 N 筆重複人員」，點進去是檢查畫面：每一組列出全部同名人員，標好哪一筆會保留、為什麼保留（已連結部屬 ／ 資料最完整 ／ 最早建立），以及每一筆各自有什麼（照片、生日、關係、作品、記事、建立日期）。",
            "【說明】刻意不做自動合併。合併不可逆，而且「同名」不保證是同一個人——公司裡真的有兩位同名同事是有可能的，那一組可以單獨關掉不合併。檢查畫面會把其他部門的重複也一併列出來，可以一次處理完。",
            "【說明】合併時是把要移除那幾筆身上「保留那筆沒有的東西」搬過去：照片、生日、職稱、職等、名片連結、部屬連結補空缺，備註兩邊接起來，子女、關係與作品併入（關係依對象去重，不會變成同一個人出現兩次）。其他人指向被移除者的人際關係、部門主管名單、名片反向連結也都會改指到保留的那一筆。",
            "【說明】被移除那筆的照片只有在「沒有被保留那筆接收」時才會真的刪檔，不會發生剛接過去就把檔案刪掉的情況。"
        ]),
        ChangelogEntry(version: "25.393", build: 1146, date: "2026/08/19", notes: [
            "【新增】機台時間軸現在會一併列出**直接上下游機台**的關聯任務／報告／會議，兩邊互相看得到：上游那台掛的東西，下游這台也看得到；反過來也一樣。每一筆都標著來源，例如「上游・M607」，一眼分得出這是隔壁那台的事、不是這台自己的。",
            "【說明】這是顯示時推導出來的，沒有真的把關聯寫進對方的資料裡。寫進去的話會有三個問題：解除上下游關係後被抄過去的關聯會變成孤兒、打開那筆任務會看到自己沒選過的機台、而且 A→B→C 會一路往下傳染到最後每台都掛滿別人的東西。",
            "【說明】只看直接相鄰一層。整條製程鏈全撈的話，線上每一台機台看到的清單會長得一模一樣，反而失去「這台附近發生什麼事」的意義。",
            "【新增】時間軸上的關聯項目下面多了一個摺疊鈕，展開看得到這一筆的完整內容：任務的內容、截止、處理措施與回復結果、完成時間、備註；報告的分類與備註；會議的時間、長度、備註與**全部議程項目**（含勾選狀態，收合時直接顯示「議程 3/5」）。",
            "【美化】關聯項目的來源機台、負責人、完成狀態改成獨立一行並且會自動換行。原本全部擠在最上面那一排，加上來源機台之後在窄螢幕上一定爆版。"
        ]),
        ...
        ChangelogEntry(version: "25.391", build: 1144, date: "2026/08/19", notes: [
            ...
            "【說明】會議的關聯掛在整場會議上，不是逐個議程項目——「這場會議在談哪幾台機台」是整場的屬性，逐項去掛的話每次開會都要重設一遍。",
            "【新增】機台詳情的時間軸（原本叫「PM／警報時間軸」）現在會一併列出關聯到這台機台的任務（青）、報告（紫）、會議（靛），照時間順序和 PM、警報混在一起排。每一筆顯示負責人，完成的標「已完成」。這樣從機台這邊就看得到完整的來龍去脈，雙向都查得到。",
            "【說明】時間軸上任務用截止日排序（沒設截止日就用任務日期），報告與會議用它們自己的日期。會議沒有「完成」的概念，議程項目全部勾完才算結案。",
            "【新增】報告與會議的卡片、分享文字也會列出關聯機台；任務卡片原本就有，現在會列出全部（多台）。",
            "【修正】刪除機台時，任務／報告／會議指著它的關聯會一併清掉。畫面本來就會略過查不到的機台所以不會壞掉，但那些失效的關聯會一直留在存檔與 iCloud 同步裡。順帶把刪除機台的整段寫入包成一次存檔，原本會連續觸發好幾次存檔與雲端推送。"
        ]),
        ChangelogEntry(version: "25.390", build: 1143, date: "2026/08/19", notes: [
            "【修正】公司組織的部門所屬設備，A→B→C 這種兩層以上的鏈，C 會整台從清單消失。上一版只往下收一層：C 因為「有上游所以不算最外層」被排除，但又沒有任何地方把它展開出來，等於憑空不見。",
            "【新增】現在是完整的樹，有幾層就展幾層。每一台有下游的機台都有自己的「下游機台 N 台」膠囊，一層一層點下去，每往下一層再縮排一點（縮太深會停止繼續往右推，免得名字被擠成一個字一行）。",
            "【說明】收合某一台時，它底下的子孫是整批隱藏，不會被丟到清單最後面。上游在別的部門、或被課別／負責人篩選擋掉的機台仍然留在最外層；資料若出現 A→B→C→B 這種環，環裡的機台會自己列在最後，不會消失。同一台機台是兩台的下游時只列一次。",
            "【新增】機台詳情卡片右上角多了「複製」。按下去會開一張預先填好的新增表單——名稱、系統別、備註、所屬部門、負責人，連上下游關係都一樣，改個名字存檔就好（名稱刻意原樣帶入，通常只要改一兩個字，不另外加「複本」後綴）。",
            "【說明】PM 與警報記錄不會跟著複製。那是原本那台機台真的發生過的保養與異常，抄到新機台上等於憑空多出一份維修履歷。",
            "【說明】上下游要等存檔拿到新的機台編號才建得起來，所以複製表單裡那一段是唯讀的清單，先讓你確認有哪些會跟過來；存檔後兩邊都看得到，之後可以再調整。"
        ]),
        ChangelogEntry(version: "25.389", build: 1142, date: "2026/08/19", notes: [
            "【新增】任務／會議議程項目／報告的編輯畫面，「完成狀態」下面多了一個「強制準時完成」。打開後完成時間會押在截止的那一刻，算準時——自己太忙沒能當下按，但同仁其實是準時交的，不該讓他白白背一個逾期。",
            "【新增】已經過了截止日才打勾時，會先跳出來問一句：強制準時完成（壓線，算準時）／照實記錄逾期（押現在的時間）／取消。取消就什麼都不會改到。",
            "【說明】這個確認在部屬卡片、部屬總覽、我的行事曆上所有的打勾圈圈都會出現，共 12 處，判斷寫在同一個地方，三種項目行為一致。",
            "【說明】什麼叫「超過截止時間」？與畫面上「準時／逾期」膠囊用的是同一把尺——以「日」為單位。18:00 截止、18:05 才按下去仍然算準時，不會跳出來煩你；要隔天以後才算逾期。沒設截止時間的項目不會有這個確認，也不會有強制準時的開關。",
            "【說明】「強制準時完成」只在該項目已勾完成、有截止時間、而且真的會被判逾期時才出現。補登過的項目下次打開編輯時開關會是開著的，看得出它被壓過線。",
            "【修正】兼任職務待辦與部屬任務／議程項目是同一件事的兩面，完成時會互相同步。原本同步時是各自重新取一次「現在」，所以壓線時間寫進去後會在同步的那一步被蓋回現在的時間——強制準時完成等於完全沒作用，而且不會有任何錯誤訊息。現在兩邊押的是同一個時間。"
        ]),
        ChangelogEntry(version: "25.388", build: 1141, date: "2026/08/19", notes: [
            "【新增】健康頁的訓練紀錄，每一列的「⋯」多了「複製到現在」。同一套菜單重做一次，不用再一個動作一個動作重打——按下去就直接產生一筆時間是現在、內容一模一樣的紀錄。",
            "【說明】帶過去的是訓練內容：標題、每個動作（名稱、類型、次數、組數、負荷、距離、時間、備註）、整場時間、場地與備註。",
            "【說明】刻意不帶的是「那一次實際量到的東西」：GPS 路徑與爬升、消耗熱量，還有 Apple 健康的對應編號。那些是當天真的發生過的數據，複製到今天等於憑空多出一筆沒發生過的紀錄；Apple 健康的編號複製過去更會讓兩筆被當成同一場訓練，同步時互相打架。",
            "【說明】複製出來的那一筆時間是「現在」，所以會直接跳到清單最上面。在下面幾筆按複製時新的那一列可能不在畫面上，所以會有一行提示告訴你複製好了、跑到哪裡去了。若有開 Apple 健康同步，新的這筆也會照常寫回去。"
        ]),
        ChangelogEntry(version: "25.387", build: 1140, date: "2026/08/19", notes: [
            "【新增】家庭成員的「關係」多了一個「寵物」。選了之後表單會換成寵物的欄位：物種（狗／貓／鳥／魚／兔／鼠／爬蟲／其他，選其他可自己填）、品種、性別、已結紮、晶片號碼、毛色特徵。",
            "【說明】生日與到家日分開問，而且兩個都可以不填——中途認養的孩子常常只知道哪天到家，硬要填一個假生日會讓年齡跟著錯。",
            "【新增】家庭子功能列在「兒女履歷」右邊多了「寵物履歷」：毛小孩總覽（隻數、狗／貓／其他、紀錄數、已結紮幾隻）加上每一隻的卡片，顯示品種、年齡與「在一起多久」。",
            "【新增】點進去是寵物詳情：物種色的英雄卡（年齡、換算人類年齡、在一起多久、紀錄數）、基本資料、體重曲線、健康與生活紀錄（疫苗與驅蟲／就醫與健檢／體重／過敏／紀念時刻）、日常紀錄（餵食／睡眠）與相簿。",
            "【說明】換算人類年齡只有貓狗會顯示，其他物種一律顯示「—」。那個換算沒有公認的算法，與其編一個數字給你看，不如老實說算不出來。",
            "【說明】寵物是當成一種家庭成員做的，所以相簿、家庭事件、日常紀錄、健康紀錄、家庭待辦指派、iCloud 同步與完整備份全部沿用既有那一套，不是另外開一個獨立的寵物資料庫。體重記在「體重紀錄」裡，累積兩筆以上就會自動畫成曲線。",
            "【修正】在職涯或家庭的子頁面左右滑動會直接跳去別的大分類（例如在「部屬」往左滑會跑到家庭或健康），等於把整個第二層跳過去。畫面上第二層膠囊本來就是接在父功能右邊、排在同一條橫列裡，現在滑動照著那條線走：從子頁面滑會在子頁面之間移動，滑到最左邊的父功能（職涯／家庭）才會離開這一組。",
            "【修正】刪掉最後一個小孩（或最後一隻寵物）之後，若人還停在那一頁，膠囊列不會有任何一顆反白、內容卻照樣顯示，現在會自動退回家庭頁。"
        ]),
        ChangelogEntry(version: "25.386", build: 1139, date: "2026/08/19", notes: [
            "【新增】機台詳情頁的「關聯機台」下面再加一個「關聯任務」章節，把機台→任務這一向補起來。上一版只做到任務指向機台，從機台這邊看不到有哪些事掛在它身上，關係只通一半。",
            "【新增】兩種任務都收進來並分開標示：警報自動掛上的標橘色「警報」，處理措施或回復結果還沒填的再加一個紅色「待回報」；手動指定關聯機台的標青色。缺失任務標紅色「缺失」。未完成排在前面，已完成收進下面的「已完成 N」展開區。",
            "【新增】章節右上的＋可以直接開一筆任務並自動帶好這台機台。機台有負責人就直接指派給他，沒有負責人才會跳出來讓你挑人（可搜尋姓名、職稱、部門）。",
            "【新增】點任務會開那位部屬的明細，任務章節就在裡面，可以直接編輯或回報。每一列會顯示負責人與日期——未完成看截止日（過期顯示「逾期」），已完成看完成日，沒設截止日就看任務日期。",
            "【修正】任務列表上手動指定的機台膠囊原本點了沒反應，現在跟警報機台膠囊一樣可以點一下只看這台的任務、再點取消。機台篩選也同時涵蓋兩種關聯方式，不會再篩出空清單。"
        ]),
        ChangelogEntry(version: "25.385", build: 1138, date: "2026/08/19", notes: [
            "【修正】設好上下游之後，進機台編輯畫面按儲存會把關係整個清掉。存檔是「重建一台機台再覆蓋」，上一版漏了把上下游帶回去。已修正，而且是從資料庫現撈最新的關係，不是用打開表單那一刻的快照。",
            "【新增】機台編輯畫面加入「關聯機台（上下游）」區塊，可以直接在這裡加或移除。這一段按下去就立刻生效、不用等儲存——因為關係要同時寫進對方那一台，跟表單其他欄位「按儲存才寫回」的作法不一樣。新增機台時不會出現，要先存檔才有對象可以連。",
            "【新增】公司組織的部門所屬設備，下游機台會收進上游機台底下並縮排，用「下游機台 N 台」的膠囊展開收合（與廠區據點的重大決議同一套作法）。展開後的子機台直接點就開詳情，不用先展開再按動作。",
            "【說明】上游在別的部門、或被課別／負責人篩選擋掉時，那台機台仍然留在最外層，不會因為找不到上游就整台從清單消失。萬一資料出現 A→B→A 這種環，也會自動退回全部平鋪。",
            "【新增】任務可以指定關聯機台。編輯任務時多一個「關聯機台」選單，這位部屬負責的機台排在最前面；指定後任務列表會出現機台膠囊，任務卡片與分享文字也會顯示。",
            "【說明】手動指定的機台與「警報自動掛上的任務」是兩回事：前者只是標記這件事跟哪台機台有關，不會要求你回報處理措施與回復結果。警報任務照舊。"
        ]),
        ChangelogEntry(version: "25.384", build: 1137, date: "2026/08/19", notes: [
            "【新增】機台詳情頁的 PM／警報時間軸下面多了「關聯機台」章節，分成上游（東西從那裡流過來）與下游（產出流到那裡去），一眼看得出製程關係。",
            "【新增】每一組右邊有＋可以加：從既有機台清單挑一台（可搜尋名稱、系統別、備註），或直接新增一台機台再自動關聯——新增時會預帶目前這台的部門與負責人，不用重填。",
            "【新增】點關聯機台就直接開那一台的詳情，PM、警報、時間軸全部看得到，可以一路往下游追下去。長按可以解除關聯（只解除關係，機台與它的記錄都不會被刪）。",
            "【說明】上下游是互為鏡像的：把 B 設成 A 的下游，從 B 點進去就會看到 A 是它的上游，不用兩邊各設一次。同一對機台也不會同時是上游又是下游——改方向時舊的關係會自動解掉。",
            "【說明】機台被刪除時，其他機台指著它的關聯會一併清掉，清單上不會留下點不開的鬼影。"
        ]),
        ChangelogEntry(version: "25.383", build: 1136, date: "2026/08/19", notes: [
            "【新增】靈動島時間軸加入 iOS 系統行事曆的當日事件（綠點）。現在三種來源都在：部屬會議（橘）、我的行事曆的個人事件（青）、系統行事曆（綠），點的顏色與詳情圖示都跟著來源走。",
            "【新增】已經從 App 同步到系統行事曆的事件不會重複出現——那是同一件事的兩個分身，只留一筆（與「我的行事曆」頁面同樣的判斷方式）。第二行標籤顯示地點，沒填地點就顯示它屬於哪一本行事曆（工作／家庭／訂閱的節日…）。",
            "【新增】設定頁會顯示系統行事曆的授權狀態：還沒問過就給一顆「允許讀取系統行事曆」，已經拒絕過就直接告訴你去哪裡開。沒授權的話時間軸只會靜靜地少一整個來源，你不會知道為什麼，所以特地講明。",
            "【說明】全日事件（含整天的節日）仍然不會出現在軸上，時間軸是一條有時刻的線。",
            "【修正】個人事件換算今天的時間，改用與「我的行事曆」同一支函式，兩邊的結果保證一致。"
        ]),
        ChangelogEntry(version: "25.382", build: 1135, date: "2026/08/19", notes: [
            "【修正】展開的靈動島四邊會吃掉內容——地點「M608 ESH」的 M 被左邊圓角切掉。四個角是圓的，貼著邊放的字一定會被咬到。現在上、下、左、右都往內縮 8 點，時間軸兩端的點與時刻標籤也一起退進來。",
            "【改善】版面加高、內容加多：會議標題從 1 行放寬到 2 行（長名稱不再被截一半），內容摘要從 2 行放寬到 3 行，時間旁邊多顯示時長（45 分 / 1.5 小時）。摘要的字數上限也從 70 字放寬到 90 字。",
            "【說明】展開區總高度的系統上限約 160 點，目前估算用到約 152 點。如果你看到底部被切掉，跟我說，那代表某個裝置的實際行高比預估大。",
            "【修正】即時動態的內容有 4KB 硬上限，原本是用「8 筆 × 估計每筆 300 位元組」心算抓的，很容易失準——中文一個字就占 3 個位元組，一場議程滿檔的會議可能就逼近上限，超過的話靈動島會直接開不起來。現在改成實際編碼後量大小，超過就自動少放一筆。",
            "【修正】順帶把場次的內部識別碼從 57 個字元縮到 19 個，那是白白吃掉容量的一段。"
        ]),
        ChangelogEntry(version: "25.381", build: 1134, date: "2026/08/19", notes: [
            "【說明】靈動島「點一下開 App、長按才展開」是 iOS 定的，所有 App 的即時動態都一樣，沒有任何 API 可以改成點一下展開。要看時間軸請【長按】靈動島。",
            "【新增】既然點一下註定是開 App，那就讓它開到有用的地方：現在點靈動島會直接跳到「我的行事曆」——那正是這條時間軸的完整版，看得到全日事件、生日、任務截止這些軸上放不下的項目。鎖定畫面上的即時動態點下去也一樣。",
            "【改善】收合狀態的靈動島右側，在今天行程都跑完時原本顯示「5 場」，看不出來已經結束，而且現在軸上不只會議、「場」這個量詞也不對。改成直接顯示「已結束」。",
            "【說明】設定頁的說明文字補上點與長按的分工，免得再誤會。"
        ]),
        ...
        ChangelogEntry(version: "25.377", build: 1130, date: "2026/08/19", notes: [
            "【新增】靈動島今日行程。把今天的會議畫成一條時間軸，左端是第一場開始、右端是最後一場結束，軸上每個點就是一場會議，位置依實際時間等比落下，所以「上午擠、下午空」這種一天的形狀看得出來；橘色細線是現在時間。",
            "【新增】軸上每個點都可以按，按下去下方就顯示那一場的時間、標題、負責的部屬與議程摘要。收合狀態的靈動島顯示下一場（或進行中那場）的時間，鎖定畫面也有完整版。",
            "【說明】互動方式是系統定的，不是我們能改的：點靈動島＝開啟 App，長按＝展開看時間軸。展開區不能捲動也不能自由點擊，所以只有點能按。",
            "【說明】系統限制一個即時動態最長 8 小時，之後會自動收起來；回到 App 就會重新掛上。場次超過 8 場時只顯示從現在起最近的 8 場（展開區的寬度也放不下更多點）。今天沒有會議就不會占用靈動島。",
            "【設定】在「設定 → 靈動島今日行程」開啟。預設關閉。如果系統的「即時動態」被關掉，這裡會直接告訴你要去哪裡打開。",
            "【技術】本版新增了一個 Widget Extension 與 App Group，App 版本與擴充功能版本必須一致，所以這一版即使功能沒動也會一起跳號。"
        ]),
        ChangelogEntry(version: "25.376", build: 1129, date: "2026/08/19", notes: [
            "【修正】進階設定裡的 18 個部屬評分權重（主動性與潛力的基礎分、各項加減分）一直沒有同步到 iCloud。同一個部屬在 iPhone 與 iPad 上會算出不同的分數，而且不會有任何提示。",
            "【修正】這件事在 v25.372 之後影響更大：績效互評的最終分數現在也吃綜合分數，兩台裝置連「評分加總」的名次都可能對不上。已補進同步清單。",
            "【修正】績效互評裡「我自己評分時採用的職等」也補上同步——那個設定決定我那張票的權重，草稿階段就該兩台一致。",
            "【說明】排名票本身（life_performance_ballots）一直都有同步，職等與權重也有，這次補的是散在本機的偏好設定。更新後兩台裝置都開一次 App 完成同步即可；權重以「最後一次修改的裝置」為準，如果你之前在兩台上各調過不同的值，請確認一次哪一邊才是你要的。"
        ]),
        ChangelogEntry(version: "25.375", build: 1128, date: "2026/08/19", notes: [
            "【新增】評分加總匯出圖片時多一個「顯示評分者姓名」開關，每次匯出都會問。關掉的話「各票名次」與「占比來源」的評分者一律顯示成「評分者 A／B／C」，尚未送出的名單也只寫人數不點名，圖片標題會註明已匿名。",
            "【說明】預設是關閉的。不小心把「誰給誰打第幾名」帶出去造成的傷害，遠大於少帶出去要重匯一次的麻煩。你的選擇會被記住，但開關每次都看得到。",
            "【說明】代號是依內部識別碼排的，與姓名、職等、名次完全無關，看圖的人推不回是誰；同一張圖裡同一個評分者的代號一致，所以還是看得出「有幾個人給了他低分」這種分布。",
            "【新增】從部屬卡片匯出「績效互評排名內容」之前會先跳確認：那張圖等於把這位同仁對每位同事的排名連同他本人姓名一起公開，而且傳出去就收不回來。確認視窗也會提醒，只想看年度結果的話評分加總頁的匯出可以匿名。"
        ]),
        ChangelogEntry(version: "25.374", build: 1127, date: "2026/08/19", notes: [
            "【新增】部屬卡片的「匯出圖片」改成選單，可以選要出「部屬卡片內容」還是「績效互評排名內容」。後者出的是這位同仁自己填的那張票：每一組的名次、基礎分、乘上他的權重之後各拿幾分，頁首附他的職等、權重、占比與送出時間。",
            "【新增】評分加總頁面右上角多了匯出圖片按鈕，出的是完整的年度結果：五個看板數字、占比來源、每個職等的 100 分基準，以及每個人的正規化／原始分數、換算式與每一張票給他第幾名。",
            "【新增】匯出前可以選人。不勾就是全部，勾一個人只出那一位，勾幾個人只出那幾位；每個職等標題旁邊也有「選取本職等」。名次一律沿用完整職等裡的名次，不會因為只選幾個人就從第 1 名重編。",
            "【說明】圖太長會自動切成多張（每張最多 1600 點，相鄰兩張重疊 24 點，切在文字中間也看得完整），一次全部分享出去。",
            "【說明】那位同仁如果還沒填過排名票，會直接說明沒有內容可以匯出，並提示可以從同一個選單代填。年度取他有票的最新一年。"
        ]),
        ChangelogEntry(version: "25.373", build: 1126, date: "2026/08/19", notes: [
            "【改善】占比終於名副其實了。排名分數與綜合分數的量級本來差很多（一個是各票加權後的累計、一個是 0～100 多的評分），直接加權會讓 13.3% 實際只影響不到 5%。現在兩者各自在同一個職等內除以該職等的最高分再乘 100，每個職等的第一名都是 100 分，占比才真的等於影響力。",
            "【新增】每一列右邊變成兩個數字「正規化 / 原始」。正規化後每個職等的第一名都是 100，跨職等看不出高低；右邊那個原始值就是用來對照的——有些低職等的人實際做的事比高職等還多，看原始值才會發現。",
            "【新增】每個職等的標題底下會寫明 100 分是拿什麼當基準換算的（排名最高幾分、綜合最高幾分）。基準取整個職等、不分課別，所以切換課別篩選不會讓分數跳來跳去。",
            "【新增】展開每個人可以看到完整推導：原始分 ÷ 本職等最高 × 100 得到正規化分，再乘上占比；最下面還附一行未正規化的同一式，兩種算法一次看完。",
            "【說明】同一個職等裡如果不同課的組人數不一樣，人多的那組基礎分上限較高，正規化後仍會略占優勢——這是排名制本身的性質，不是這次正規化造成的。"
        ]),
        ChangelogEntry(version: "25.372", build: 1125, date: "2026/08/19", notes: [
            "【修正】某個職等在課裡只有一個人時，那個人從頭到尾不會出現在任何一張評分票上，評分加總也永遠看不到他。現在單人組一併列入：直接算第 1 名、基礎分 1 分。",
            "【新增】評分票在年度下面多了「排名分數占比」欄位，填你認為互評排名該占最終評分幾成。年度實際採用的是所有已送出票的加權平均，權重就是各自的職等權重——例如 31 職等(×1) 填 20%、32 職等(×2) 填 10%，平均是 (20×1＋10×2)/3 ＝ 13.3%。",
            "【新增】評分加總的最終分數改為「排名分數 × 占比 ＋ 綜合分數 ×（1 − 占比）」。綜合分數就是人才矩陣的主動性與潛力平均。名次依最終分數重排，一樣依職等分開、各自從第 1 名起算。",
            "【新增】看板多一列：排名平均占比、總分占比。底下可以展開看占比是怎麼算出來的——每張票填了多少 %、權重多少、加權平均的分子分母。",
            "【新增】展開每個人可以看到最終分數的組成：排名分數 × 占比、綜合分數 ×（1 − 占比），以及原本就有的每張票給他第幾名。收合狀態的副標也補上這兩個來源分數。",
            "【說明】還沒有人填占比時，維持 100% 採用排名分數，與這版之前完全一樣——全部留空不會被當成「排名占 0%」，那會把整頁默默換成綜合分數排名。畫面上會註明目前是哪一種狀態。",
            "【說明】已轉出或離職的人在部屬清單裡查不到，算不出目前的綜合分數，那一項以 0 計並在明細標示。兩段分數的量級本來就不同（排名分數是各票加權後的累計、綜合分數是 0～100 多的評分），數字大小不能直接對比。"
        ]),
        ChangelogEntry(version: "25.371", build: 1124, date: "2026/08/19", notes: [
            "【改善】部屬卡片的「任務」章節換成項目模板。標題不再只有一行硬切掉，長標題會完整換行；一堆膠囊（截止、缺失、分數、兼任連動、機台、系統別、建立日）改成橫向捲軸，不會再把標題擠扁。",
            "【新增】任務內容直接顯示在列表上（最多三行）。以前得點進去才知道這件任務在講什麼。",
            "【新增】備註、處理措施、回復結果收在一顆「細節 N 項」膠囊裡，點開看標題、再點一次原地展開內文——這三欄以前只存在編輯頁裡，列表上一個字都看不到。",
            "【改善】膠囊順序改成截止日、缺失在最前面。橫向捲軸只有排前面的一眼看得到，最該先看到的是快到期與缺失，不是建立日期。",
            "【修正】匯出部屬卡片圖片時，橫向捲軸捲出去的膠囊會被整個裁掉——靜態圖是捲不動的。現在出圖時膠囊自動改成換行排版，該有的標籤全部入鏡。部屬總覽與重大決議的分享圖也一起修正（同樣的問題）。",
            "【說明】打勾、點開任務、點機台／系統膠囊篩選這些操作都跟原本一樣，沒有變動。"
        ]),
        ChangelogEntry(version: "25.370", build: 1123, date: "2026/08/19", notes: [
            "【修正】部屬卡片的標題被擠成「部屬...」。右上角原本並排四顆按鈕（分享、排名、升職、編輯），加上左邊的「關閉」，標題只剩下一點點寬度。",
            "【改善】四顆合併成一顆「⋯」選單：編輯、升職、績效互評排名，分隔線下面是匯出圖片與匯出文字。標題現在完整顯示「部屬卡片」。",
            "【說明】匯出原本就要按一下分享圖示才展開，現在改成在同一個選單裡直接看到兩個選項，點的次數沒有變多。"
        ]),
        ChangelogEntry(version: "25.369", build: 1122, date: "2026/08/19", notes: [
            "【新增】信用卡綁定的一卡通卡號比照卡號，邊打邊每四碼自動補一個「-」，固定 16 碼。鍵盤也改成純數字鍵盤。",
            "【改善】已經存過的一卡通卡號不用重打：打開編輯畫面會自動幫你分好組，卡片明細也直接以分組後的樣子顯示。如果你當初在卡號裡加了數字以外的註記，會原樣保留、不會被改掉。",
            "【說明】悠遊卡沒有跟著改：它的卡號是 9～10 碼，四碼一組會切得很奇怪，維持原本的自由輸入。"
        ]),
        ChangelogEntry(version: "25.368", build: 1121, date: "2026/08/19", notes: [
            "【新增】財富的信用卡可以輸入完整卡號了，不再只能填末四碼。一路打數字就好，軟體會自動每四碼補一個「-」。支援 13～19 碼，所以 15 碼的美國運通、19 碼的銀聯卡也填得進去。",
            "【新增】存過一次之後，卡號只會顯示「•••• •••• •••• 末四碼」，到期日顯示「••/••」。要看完整內容必須先過 Face ID（沒有生物辨識會自動改問裝置密碼），看完 30 秒自動收回去，離開頁面也會立刻上鎖。",
            "【新增】檢核碼（CVV／CVC）完全由你決定要不要記，預設不記。它是網路刷卡唯一還缺的那一塊，能不留就不留——這是刻意做成預設關閉的。",
            "【安全】完整卡號與檢核碼一律加密後才存（AES-GCM），金鑰放在系統鑰匙圈、透過 iCloud 鑰匙圈端對端同步到你自己的其他裝置。密文會跟著資料同步，但金鑰不會進 App 的資料庫、不會進備份檔、桌面網頁版也拿不到——沒有金鑰，那串密文就只是亂碼。",
            "【說明】如果換了裝置卻沒開 iCloud 鑰匙圈，新裝置會解不開舊卡號，這時會直接告訴你原因，你可以重新輸入一次。既有只填末四碼的卡完全不受影響，照舊顯示。"
        ]),
        ChangelogEntry(version: "25.367", build: 1120, date: "2026/08/19", notes: [
            "【新增】廠區編年史出圖時會顯示進度浮層，一步一步告訴你現在在做什麼：量測版面、繪製、壓縮、存檔，附一條實際的進度條。樣式沿用設定頁那幾顆系統按鈕的圖示圓規格。出圖期間畫面會擋住，再按一次也不會重複執行。",
            "【新增】圖如果超過螢幕長邊的 1.5 倍，自動改出 PDF。這種尺寸的圖本來就不該是點陣圖——PDF 是向量的，沒有像素上限，放到多大都清楚，列印也漂亮，而且直接寫進檔案、不吃記憶體。小張的仍然出 PNG，方便直接貼到聊天室。",
            "【改善】壓縮檔案移到背景執行，主畫面不再整個卡住。壓縮方式也改成邊壓邊寫進檔案，不像以前要先在記憶體裡組出一整份完整結果——這正是前一版「圖片編碼失敗」的來源。",
            "【改善】量尺寸不再先畫一張完整的圖出來。以前光是「量這張圖有多大」就要配置上百 MB 的記憶體，現在只量不畫，快很多也省很多。",
            "【改善】PDF 單頁有 14400 點的系統上限，超過會做出打不開或被裁掉的檔案。現在會自動把每個廠逐條列出的決議數縮到放得下為止（多的照樣寫成「還有 N 則決議」），真的放不下才會告訴你要改用橫式。"
        ]),
        ChangelogEntry(version: "25.366", build: 1119, date: "2026/08/19", notes: [
            "【修正】廠區編年史出現「圖片編碼失敗」。原因是圖太大：據點與決議多的時候，直式會做出上萬像素高的圖，畫是畫得出來，但要把它壓成 PNG 檔時記憶體不夠，系統只是安靜地放棄，所以只看得到一句沒頭沒尾的「編碼失敗」。",
            "【修正】現在出圖前會先量好整張圖的尺寸，再決定用多高的解析度，確保落在這台裝置能處理的範圍內（單邊 10000 像素、總共 2400 萬像素以內）。",
            "【修正】如果連最低解析度都還是太大，會自動減少每個廠逐條列出的決議數（10 則→5 則→2 則），多出來的照原本的做法寫成「還有 N 則決議」，資訊不會不見，只是不逐條列。",
            "【修正】PNG 真的壓不出來時改存 JPEG（同樣清晰度下需要的記憶體少很多），而不是直接失敗。",
            "【改善】萬一還是失敗，錯誤訊息會直接告訴你這張圖是幾乘幾像素、以及可以改用另一個方向（直式／橫式）試試，不再只說「編碼失敗」。"
        ]),
        ChangelogEntry(version: "25.365", build: 1118, date: "2026/08/19", notes: [
            "【新增】變動支出的「汽車」分類現在可以選地點了。停車、充電、加油、保養、洗車、維修都會多出一列地點欄，輸入時會跳出 Apple 地圖的候選與你之前去過的地方，選到就一併記下座標——這是之後要做汽車里程路線圖的基礎。",
            "【說明】汽車的名稱欄被「項目 N：型號-支出類別」的自動命名佔住，沒地方放店名，所以地點是獨立的一列，不會蓋掉自動命名。其他有地點的分類（飲食、娛樂、購物、日用品、醫療）維持原本作法，店名就是名稱欄。",
            "【新增】地點的候選清單會依支出類別分開：選「停車」只會跳出你停過的停車場，不會混進加油站與保養廠；還沒連結車輛時則不分類別，全部一起給。",
            "【新增】變動支出列表上，汽車那一筆會在分類標籤後面用青色顯示地點名稱（自動命名的標題看不出停在哪、在哪充電）；點進去的明細卡片與分享文字也多了「地點」一行。搜尋也吃得到地點名稱。",
            "【修正】原本選好餐廳之後再把分類改掉，那組座標會跟著被存進新分類（例如變成一筆有餐廳經緯度的停車費）。現在會記住座標是在哪個分類底下選的，分類一改就不採用，避免地圖標到不相干的位置。"
        ]),
        ChangelogEntry(version: "25.364", build: 1117, date: "2026/08/19", notes: [
            "【修正】按「廠區編年史」沒有任何反應。出圖失敗時程式只是安靜地不做事，畫面上什麼都沒有——這是最糟的處理方式。現在失敗一定會跳出說明，告訴你是哪一種原因。",
            "【修正】直式的時間軸連接線原本用「填滿剩餘高度」的寫法，橫式的整張圖沒有指定寬度，兩者都可能讓系統量不出圖片尺寸而出圖失敗。連接線改畫在背景（拿得到實際高度），整張圖一律先算好寬度再畫。",
            "【調整】出圖倍率由 3 倍改為 2 倍。據點與決議多的時候 3 倍容易做出超大點陣圖而失敗，2 倍貼簡報或列印仍然夠清楚。",
            "【說明】如果你的據點還沒填「啟用年月」，編年史會排不進時間軸——現在會直接告訴你要去補，而不是按了沒反應。"
        ]),
        ChangelogEntry(version: "25.363", build: 1116, date: "2026/08/19", notes: [
            "【新增】公司組織右上角的分享鈕改成選單，除了原本的「組織圖 PDF」，多了「廠區編年史」，直式與橫式各一種，出的是高解析 PNG。",
            "【新增】編年史把每個廠依啟用時間由早到晚串成一條時間軸：圓點標出啟用年月與期間，旁邊是廠名、公司、地點與年資，底下依時間列出那段期間的重大決議（日期、流水號、標題、系統分類與發起人）。直式適合決議多的情況，橫式一個廠一欄、適合廠多決議少。",
            "【說明】每個廠最多畫 10 則決議，超過的會寫「還有 N 則」，避免圖片長到不能看。沒有填啟用年月的據點不會出現在年表上——排不進時間軸。",
            "【說明】圖是用襯線字體的紙張風格排的，右下角有匯出日期。全部由程式繪製，不含任何外部素材。"
        ]),
        ...
        ChangelogEntry(version: "25.360", build: 1113, date: "2026/08/19", notes: [
            "【新增】公司組織頁多了一條固定在頂端、不隨清單捲動的「決議搜尋」列（有建廠區據點時才出現）。輸入關鍵字會搜尋所有重大決議的標題、內容、廠區、發起人、流水號與系統分類，然後把清單直接捲到那筆決議所屬的廠區，該廠會自動展開、命中的那筆決議標亮並展開內文，不用自己一個一個找。",
            "【新增】命中多筆時像 Word 一樣可以逐一跳：搜尋列右側顯示「第幾筆／共幾筆」，上下箭頭往前往後切換，到底會繞回第一筆。下方會顯示目前跳到哪一筆決議的標題。",
            "【說明】這條搜尋列只在目錄模式出現（樹狀圖沒有廠區清單可捲）；原本的人員／報告／改善項目搜尋不受影響，兩者分開。"
        ]),
        ChangelogEntry(version: "25.359", build: 1112, date: "2026/08/19", notes: [
            "【修正】點「新增據點」後畫面打開又立刻收起來。原因是新增／編輯的視窗掛在清單的章節上，清單一重建章節內容就把剛打開的視窗一起收掉了。改成掛在公司組織頁本身，開了就會留著。",
            "【新增】公司名稱填過一次之後會變成膠囊，下次點一下就帶入，不用重打；個人檔案裡的公司也會一起列出來。同一家公司好幾個廠的情況省很多打字。",
            "【新增】地點同樣會記住填過的值並變成膠囊，也獨立成一個區塊——同一個園區常有好幾個廠。",
            "【說明】膠囊點一下帶入、再點一次清空，與重大決議的廠區／發起人膠囊操作一致。"
        ]),
        ChangelogEntry(version: "25.358", build: 1111, date: "2026/08/19", notes: [
            "【修正】決議掛在哪個廠改成看時間，不是看廠名。規則：一筆決議屬於「啟用年月早於或等於這筆決議日期」之中最晚的那個廠。A 廠蓋好之後才發生的事，如果那時 B 廠已經啟用，就算在 B 廠那個時間點上——A 廠沒有發生這件事。每個廠的期間到下一個廠啟用為止，最後一個廠算到今天，串起來就是一條連續的年線，膠囊上會直接顯示各自的期間。",
            "【新增】章節右上角一顆排序鈕，按一下換一種，三種循環：時間 新→舊（預設）→ 時間 舊→新 → 廠名 A→Z → 回到時間 新→舊。選擇會記住，下次打開還是同一種。",
            "【說明】日期早於你建檔的第一個廠的決議，會集中在最下面的「最早的據點之前」那一列，不會憑空消失；補一個更早的據點就會自動歸進去。沒填啟用年月的據點不參與分期。",
            "【說明】決議上的「廠區」文字欄位保留不變，只是不再決定歸屬。章節下方那份「決議裡出現過、還沒建檔」的名單也還在，方便你照著既有決議把廠區一個一個補建起來。"
        ]),
        ChangelogEntry(version: "25.357", build: 1110, date: "2026/08/19", notes: [
            "【新增】公司組織目錄最下面新增「廠區據點」章節。一家公司有很多廠的情況（像台積電的 F12A、F14、F18）可以逐一建檔：公司、廠區名稱、地點，以及啟用年月（只選年跟月）。已經關廠或轉移的可以填結束年月，那筆會淡化但資料保留。",
            "【新增】廠區與重大決議自動連結，不用另外設定。決議上本來就有「廠區」欄位，只要填的字串對得上廠名（或公司＋廠名），那筆決議就會掛到這個廠底下。點開廠的「重大決議 N 項」就是這個廠的歷史年線：依日期排好，點標題展開看內容，再點「開啟這筆決議」直接跳到決議卡片。比對時會忽略前後空白與英文大小寫。",
            "【新增】如果決議上填過某個廠區、但還沒建成據點，章節下方會列出那些名稱並提醒你，點一下就會帶著名字開新增頁，省得重打。",
            "【說明】版型用的是 v25.354 的共用項目模板——項目是廠，子項目是決議，跟重大決議那頁的操作手感一致。刪除據點只會刪掉據點本身，掛在底下的決議不受影響。"
        ]),
        ChangelogEntry(version: "25.356", build: 1109, date: "2026/08/19", notes: [
            "【調整】重大決議取消左滑刪除。決議是定案紀錄，滑一下就沒了風險太高。要刪除請點進該筆決議 → 編輯 → 最下方的「刪除此決議」，多一道確認的步驟。",
            "【說明】同一頁的待辦、成員、會議紀錄、重要日期維持左滑刪除不變——那些項目重建成本低，決議不一樣。"
        ]),
        ChangelogEntry(version: "25.355", build: 1108, date: "2026/08/19", notes: [
            "【調整】部屬總覽的任務、請假、報告三種列改用 v25.354 的共用版型，跟重大決議一致：標籤全部收進可橫向捲動的膠囊列，標籤再多也不會把標題擠掉；標題完整顯示、內容預覽限行數。左側的漸層圓圈維持原本的功能（任務與報告點一下就是完成／取消完成）。",
            "【調整】原本擠在標題旁邊的人員膠囊移到膠囊列裡，點一下一樣是以這個人篩選、再點一次取消；報告的類型與逾期狀態徽章也一併移進膠囊列。請假列的人名仍然是標題，點人名照舊可以篩選。",
            "【說明】會議列這次沒有動——它的議程子項目更適合改用版型的摺疊子項目，會在下一版單獨處理。"
        ]),
        ChangelogEntry(version: "25.354", build: 1107, date: "2026/08/19", notes: [
            "【調整】把重大決議那套列表版型抽成全 App 共用的模板，之後其他區塊可以逐步換過來，版型與操作手感就會一致：膠囊列橫向捲動不擠壓標題、標題完整換行、預覽限行數、子項目預設收合成一顆計數膠囊、點開列出標題、再點一次在原地展開內文。",
            "【說明】模板留了插槽因應各區塊的差異：左側可放勾選框或日期塊、右側可放分數／金額／選單、標題下方可加進度條，另有一個自訂插槽放地圖或迷你圖表之類的東西。重大決議已經改用這套模板，畫面與操作與先前完全相同。",
            "【說明】這一版只換了重大決議一處，其他區塊維持原樣。之後每換一個區塊都會是獨立的一次更新，才不會一次動到太多畫面。"
        ]),
        ChangelogEntry(version: "25.353", build: 1106, date: "2026/08/19", notes: [
            "【改善】兼任職務的重大決議列表，參照前案改成可摺疊的子項目。預設只顯示一顆「參照前案 N 項」膠囊，點開列出每個前案的流水號、標題與日期，再點標題就在原地展開看那筆前案的內文——不用跳到別的畫面，在清單裡就能把來龍去脈看完。展開後還有「開啟這筆決議」可以直接跳過去。",
            "【改善】清單的內容預覽不再被舊格式的引用文字佔滿。v25.318 以前的版本會把前案的標題與內文直接貼進決議的內容欄位，所以舊的決議在清單上會先看到一段灰色的「── 參照前案 …」。現在預覽會自動略過那一段，只顯示你本次真正寫的內容。",
            "【說明】那段舊文字仍然留在內容欄位裡（沒有自動改你的資料）。想真的清掉的話，編輯該筆決議時「內容」區塊下方會出現「整理舊格式」按鈕，按一下就會把重複的引用段落移除；前案本身不受影響，仍然在上面的參照清單裡。匯出的文字與圖片一樣是完整內容，不受預覽調整影響。"
        ]),
        ChangelogEntry(version: "25.352", build: 1105, date: "2026/08/19", notes: [
            "【新增】跑步路徑紀錄：健康頁右上角的跑者圖示，選跑步／健走／騎車／登山後按開始，邊跑邊在地圖上畫出路徑，即時顯示距離、時間、配速與累計爬升。可暫停再繼續，暫停期間不算時間也不會把那一段連成直線。結束後直接存成一筆訓練，紀錄裡就能看到當時的路線圖與每段數據。",
            "【新增】背景記錄：鎖屏或切到其他 App 也會繼續記，狀態列會出現藍色指示條。回到健康頁時上方有一條橘色橫幅顯示目前距離與時間，點一下回到記錄畫面。",
            "【新增】Apple 健康聯動（健康頁上的開關）。打開後：Apple 健康裡的體重、血壓、心率會帶進健康檔案（同一天／同一分鐘已經有的不重複加），身高沒填過會自動補；Apple Watch 或其他 App 記的訓練會帶進訓練清單，有 GPS 的還會一併帶路徑；你在這裡記的訓練也會寫回 Apple 健康，計入體能訓練圓環。第一次同步往回抓 90 天，之後只抓上次同步之後的。",
            "【說明】HealthKit 沒有「組數／次數／負荷」這種欄位，這是 Apple 的限制不是本 App 的。所以重訓寫回去只會是一段時間與消耗熱量，Apple 健康與其他 App 看不到你的 20 下 3 組；從 Apple Watch 匯入的重訓也只有時間與熱量，動作明細要自己補（匯入的紀錄會標註提醒）。體重與血壓的聯動則是完全對等的。",
            "【說明】路徑點在記錄當下就抽稀（至少移動 8 公尺或間隔 5 秒收一點，單次上限 3000 點），一趟 5 公里大約只佔 15KB，不會把 iCloud 同步撐大。桌面網頁版讀不到 Apple 健康，但看得到已經寫進健康檔案的體重與血壓。"
        ]),
        ChangelogEntry(version: "25.351", build: 1104, date: "2026/08/19", notes: [
            "【新增】人生分頁在「財富」右邊多了「健康」頁，只要有健康里程碑或健身紀錄就會出現。右上角「＋」記錄一次訓練：先給這次訓練一個名字（例如胸推日），再往下加動作。",
            "【新增】一次訓練可以掛多個動作，動作分兩種型態。重量訓練填「每組次數、組數、負荷」，負荷可選自身體重／負重（填公斤）／徒手，例如伏地挺身 20 下、3 組、自身體重；有氧填距離與時間，配速自動算。動作名稱會記住你用過的，之後點一下就帶入。",
            "【新增】兩筆以上就會畫「綜效曲線」，可切換三種看法：總訓練量、總次數、單一動作。訓練量＝每組次數 × 組數 × 負荷；負荷是「自身體重」時會帶入醫療地圖健康檔案裡最近一次的體重，所以體重變了曲線也跟著反映。還沒有體重紀錄時，自身體重的動作只計總次數、不會灌進總訓練量（不亂猜一個數字），頁面上會提醒你去補一筆體重。",
            "【新增】看板顯示本月訓練次數、本月訓練量、連續訓練週數與累計次數；下方「動作統計」列出每個動作練過幾次、幾組、幾下與最佳表現（最重幾公斤／最長幾公里／單組最多幾下），點一下直接跳到那個動作的進步曲線。",
            "【新增】健康頁下方會一併列出所有「健康」分類的里程碑，訓練紀錄與人生大事看得到彼此。"
        ]),
        ...
        ChangelogEntry(version: "25.349", build: 1102, date: "2026/08/19", notes: [
            ...
            "【改善】某人被歸在哪個職等，是看票上的快照而不是他現在的職等——轉出課別或離職的人一樣會留在當年被評的那個職等裡，歷年結果不會因為之後升職而變動。",
            "【改善】課別篩選會記住上次的選擇，下次進來直接停在同一課，不用每次重選「全部課別」；如果那個課別已經被刪掉，會自動回到全部。",
            "【改善】名次列不再重複顯示職等（段落標題已經有了），改成只顯示課別、票數與平均名次。"
        ]),
        ChangelogEntry(version: "25.348", build: 1101, date: "2026/08/19", notes: [
            "【新增】年度績效互評：同仁互相排名，排名自動換算成分數。入口有兩個——部屬總覽看板的獎盃鈕（蛋糕左邊）填「我的評分」，部屬卡片右上角的獎盃鈕填「該同仁的評分」；今年已送出時獎盃會亮橘色。",
            "【新增】評分方式是拖曳排序：名單依「同課 × 同職等」分組，每組至少 2 人才需要排，評分者自己也在組內（含自評）。拖動時即時顯示每個人會拿幾分，送出前就看得到結果；送出後仍可修改再更新，中途離開也會保留草稿。",
            "【新增】部門職等頁的每個職等多了橘色「×」績效權重欄（留空＝×1）。計分規則：某組 N 人時第 1 名基礎分 N、往下每名少 1 分，再乘上**評分者職等**的權重後全部加總。例：32 職等組 3 人，31 職等（×1）投的第一名得 3 分，32 職等（×2）投的第一名得 6 分——職等越高，票越重。",
            "【新增】人才矩陣新增第三個分頁「評分加總」：年度切換、課別篩選、總排名表（含票數與平均名次），點任一人展開可看完整分數來源——誰給了第幾名、基礎分多少、乘上多少權重、小計多少，並列出還沒送出票的人提醒催繳。",
            "【說明】評分者的職等與權重在送出當下存成快照，日後升職不會回頭改寫舊年度的分數；被評分者的姓名也存快照，轉出課別或離職後分數完全保留，新人明年會自動出現在名單裡。各課人數不同、基礎分上限就不同，跨課比較僅供參考。"
        ]),
        ChangelogEntry(version: "25.347", build: 1100, date: "2026/08/19", notes: [
            "【新增】固定支出可以「停止」了：編輯頁多了「停止」區塊，打開開關、填最後一次扣款日與原因（取消訂閱／合約到期／貸款繳清／保單期滿或解約／搬遷退租／換方案／資產已出售）即可。不用刪除項目——過去已經發生的期數與統計完全保留，只是從結束日之後不再扣款。",
            "【說明】計算規則：排定扣款日在「最後一次扣款日」當天或之前的期數才算。例：8/15 起每月扣，結束日填 9/14 只算 1 期；填 9/16 算 2 期。停止後不再計入月固定支出、銀行餘額扣款、信用卡消費、圖表與節稅金額；年中停掉的項目只算到停止的那個月，不再整年灌進節稅。編輯時會即時顯示「實際扣款期數與合計」。",
            "【修正】已繳完的貸款過去會永遠扣下去：固定支出原本沒有結束日，展開一律跑到今天，所以繳清多年的房貸／車貸每個月仍在扣銀行餘額、仍計入月固定支出，連歷史曲線都是錯的。這版依「貸款年限」自動推算最後一期並補上結束日（推算日已過的標記為「貸款繳清」），提前清償或轉貸可自行修改；已經填過結束日的不會被動到。",
            "【新增】固定支出列表：已停止的項目顯示橘色原因徽章、整列淡化並沉到各分類最後，不再影響分類排序；卡片加上「最後扣款」與「實際期數」。摘要與圖表只算進行中的項目。",
            "【網頁版】同步支援結束日（v1.4）：固定支出頁分「進行中／已停止」，已停止顯示止於哪天與原因、不計入每月等值，並多一個「已停止・每月省下多少」的看板；總覽的 30 天內到期也不再列出已停止的項目。"
        ]),
        ChangelogEntry(version: "25.346", build: 1099, date: "2026/08/19", notes: [
            "【新增】會議卡片每一個場次的右側多了「＋」：不用進編輯頁，直接為那一場新增議程項目（內容可 @ 標註、指派負責人、截止日期、備註）。加開的臨時場次、規則推導出的場次、已有議程的場次都有；已取消的場次不給加。不分場次的會議在「議程項目」區塊右側也有「＋」，還沒有任何議程時會先顯示一個空區塊讓你加第一項。",
            "【說明】新增的項目直接落在該場次名下，評分依指派的負責人計算（沒指派才計給會議負責人）。"
        ]),
        ...
        ChangelogEntry(version: "25.342", build: 1095, date: "2026/08/19", notes: [
            "【網頁版】側欄改為與 App 相同的四大分類與順序：收支（總覽／收入／變動支出／固定支出／圖表）、理財（總覽／儲蓄險／股票／載具／房地產／圖表）、人生（總覽／履歷／財富／職涯：我的行事曆、部屬總覽、部屬、人才矩陣、統計圖表、名片、公司組織、部門職等、兼任職務／家庭／房地產／稅務／三張地圖）、設定。尚未提供網頁版的項目以灰色列出，順序照 App。",
            "【網頁版】收支五頁全部上線：總覽（本月與今年 KPI、近 12 個月收支、分類、最近記帳、30 天內到期固定支出）、收入（週期／單次、年度篩選）、變動支出（逐月瀏覽、分類膠囊附金額、搜尋、付款方式、充電度數）、固定支出（每月等值、下次扣款日、金額變動、貸款資訊）、圖表（年度收支、逐月收支、年度分類）。",
            "【網頁版】理財六頁：總覽（資產配置條、持股前五、銀行帳戶）、儲蓄險（期間進度、外幣台幣等值）、股票（點入明細：現金股利累計、含息報酬、交易紀錄、配息配股）、載具（價值折舊、每月固定、變動支出分類、電動車充電 KPI 與每度電價走勢）、房地產（房貸餘額與進度、已付價金、變動支出、電梯保養）、圖表（資產配置圓餅、持股市值與成本）。人生・財富＝財富卡片（銀行最近存提、信用卡最近消費、證券持股）。設定頁可更換 Token、登出、調整自動更新間隔。原「財務總覽」網址自動導向新頁面。App 本身無變動。"
        ]),
        ChangelogEntry(version: "25.341", build: 1094, date: "2026/08/19", notes: [
            "【網頁版】新增「財務總覽」：淨資產概算（銀行＋股票＋保險＋房產＋車輛 − 房貸餘額）、銀行總餘額、股票市值與損益、本月支出／收入與信用卡消費、保險房產車輛合計五個 KPI；總覽分頁有近 12 個月收支長條圖與支出分類圖、銀行帳戶餘額列表（多幣別顯示各幣別）、信用卡本月消費與額度使用率。",
            "【網頁版】帳戶分頁列出全部財富卡片（銀行／信用卡／證券／保險）；股票分頁列持有中與已賣出（成本、市值、損益、報酬率、配息次數，美股依匯率表「美金」換算）；保險・房產・車輛分頁列儲蓄險（含期間進度）、房地產（含房貸餘額）、車輛。",
            "【說明】銀行餘額計算規則與 App 財富卡片相同：手動存提款＋固定支出逐期展開＋週期收入展開＋連結信用卡消費扣款，外幣依匯率表換算。新增讀取記帳、收入、匯率、儲蓄險、股票、車輛、房地產七份資料。App 本身無變動。"
        ]),
        ChangelogEntry(version: "25.340", build: 1093, date: "2026/08/19", notes: [
            "【網頁版】修正行事曆同一個人的生日在同一天出現兩次（月曆範圍沒跨年時年份被算了兩遍）。App 本身無變動。"
        ]),
        ChangelogEntry(version: "25.339", build: 1092, date: "2026/08/19", notes: [
            "【網頁版】新增「我的行事曆」：月曆格子＋右側當日清單。彙整部屬任務（截止日）、會議場次（含週期展開、改期、臨時場）、議程項目截止、報告、部屬請假（跨日逐天顯示）、兼任待辦截止、兼任重要日期與會議、個人事件（含每天／每週／每月／每年重複）、部屬與組織人員生日。逾期的顯示紅色、已完成的劃掉。",
            "【網頁版】行事曆頂部可依類別開關顯示（各類別附本月數量），上月／今天／下月切換，點任一天看當日清單，點項目直接跳到對應的部屬或兼任職務頁。網址帶年月日可直接開到指定日期。App 本身無變動。"
        ]),
        ChangelogEntry(version: "25.338", build: 1091, date: "2026/08/19", notes: [
            "【網頁版】新增「兼任職務」頁：列表卡片顯示主責／協辦、在任／已卸任、就任卸任日、下個重要日期、待辦完成進度與決議／會議／成員／重要日期數。點進工作區有五個分頁：待辦（負責人可連到部屬、系統分類、連結的部屬任務／議程）、重大決議（流水號、廠區、分類、發起人、參照前案可展開看全文、被後案參照的回連；可搜尋流水號／標題／內容，可依分類篩選）、會議紀錄、成員（連結部屬可點入、各自待辦完成數）、重要日期（即將到來／已過、提前提醒天數）。App 本身無變動。"
        ]),
        ...
        ChangelogEntry(version: "25.335", build: 1088, date: "2026/08/19", notes: [
            "【網頁版】統計圖表的「團隊平均」灰色長條改依數值排進序列（不再固定放最下面），一眼看出誰在平均之上／之下。",
            "【網頁版】登入 iCloud 後每 10 分鐘在背景自動重新讀取；只有資料真的有變才重畫並提示，看到一半不會被打斷。分頁切到背景時暫停、回到前景若已過期立刻補抓；左下角顯示讀取時間與下次自動更新時間，「重新讀取」仍可隨時手動更新。App 本身無變動。"
        ]),
        ChangelogEntry(version: "25.334", build: 1087, date: "2026/08/19", notes: [
            "【調整】會議評分規則改為「掛名有基本分、議程項目看指派」：會議負責人每掛名一場會議固定 +1（評分權重可調「會議掛名（每場）」）；議程項目完成的 +1 改計給項目上指派的負責人（多人各計一次），沒指派、或負責人都不是部屬（外部名片／組織人員）才計給會議負責人。原本項目分數一律算給會議負責人、指派的人一分都沒有。",
            "【調整】評分明細對應改為「會議掛名 ×N」與「完成議程項目（指派給我）×N」；部屬列表、人才矩陣、統計圖表、明細頁、匯出文字全部同步採用新規則。指派給部屬對應的組織人員或名片，也會反查回該部屬計分。",
            "【同步】桌面網頁版的評分移植同步更新為相同規則。"
        ]),
        ...
        ChangelogEntry(version: "25.332", build: 1085, date: "2026/08/19", notes: [
            ...
            "【說明】會議議程項目的完成分（+1／項）目前全部計給「會議負責人」，項目上指派的負責人（例如 B 同仁）不加分、也不會兩人都加；若要改成「計給項目負責人」請告知，這是評分規則的變更。"
        ]),
        ChangelogEntry(version: "25.331", build: 1084, date: "2026/08/19", notes: [
            "【修正】事項卡片（任務／會議／報告／請假／紀錄）右上角「編輯」帶入的是開卡片當下的舊快照：若已在卡片上用鉛筆改過內容或備註、或已進編輯頁存過一次，再按「編輯」表單仍是舊值，一存檔就把先前的修改整個蓋回去——看起來就像「儲存了但卡片沒更新」。現在編輯頁一律帶資料庫裡的最新內容。",
            "【新增】任務卡片補顯示「任務分數」（狀態列下方）：一般任務顯示 +N 分（預設／自訂），缺失任務顯示扣分——之前在編輯頁改分數，卡片上完全看不到變化。",
            "【修正】編輯既有「應做未作為（缺失）」任務時，打開表單會把已存的自訂分數（例如 -3）自動洗回 -1；現在只有親手切換開關才會重設分數。"
        ]),
        ChangelogEntry(version: "25.330", build: 1083, date: "2026/08/19", notes: [
            "【修正】名片放大檢視「圖片超大、只剩中間、找不到關閉鈕」的真正根因：畫面底下的模糊背景圖會把整個檢視器撐到比螢幕還寬（橫式名片約撐到 1440pt），整層置中後左右溢出——前景照片因此「貼合」到超寬的容器，看起來像放大只剩中間；左上角的關閉鈕也被推到螢幕外（使用者猜得沒錯）。上一版只換按鈕顏色是誤判，抱歉。",
            "【修正】背景圖改以「佔位層＋覆蓋」方式排版，只吃螢幕尺寸、不再被照片撐大：開啟即整張名片完整貼合（橫式以寬度貼齊），關閉鈕回到左上角，下滑關閉、雙擊／雙指縮放照常。所有用到此檢視器的地方（名片、美食／旅遊／就醫地圖、育兒相簿、通用照片廊）一併修正。"
        ]),
        ChangelogEntry(version: "25.329", build: 1082, date: "2026/08/19", notes: [
            "【修正】名片放大檢視的關閉按鈕改為高對比深色圓鈕（白色描邊）：原本的半透明按鈕在白底名片照上幾乎隱形，看起來像「沒有地方可以關閉」。",
            "【新增】放大檢視支援「下滑關閉」手勢：未縮放時往下拖曳圖片會跟著手指移動，放開超過門檻即關閉——找不到按鈕也關得掉。",
            "【說明】開啟時整張名片會完整貼合畫面（橫式名片以寬度貼齊、上下留黑），畫面周圍的模糊底圖只是背景裝飾；若看到「只有中間放大」多半是先前誤觸雙擊放大所致——雙擊可在貼合／放大間切換，雙指縮放亦可。"
        ]),
        ChangelogEntry(version: "25.328", build: 1081, date: "2026/08/19", notes: [
            "【新增】變動支出卡片會顯示充電資訊了：關聯汽車的電費支出若填了度數／電量 %／里程錶，卡片多一塊「充電資訊・車名」——充電度數、電量區間（含 +Δ%）、里程錶、每度電價、本次推估電池容量（充電區間 ≥15% 才算）。",
            "【新增】卡片內附「每度電價走勢（這輛車）」小圖：這輛車全部充電紀錄的電價曲線，本筆以紅色大圓點標示——當下就看得出這次充貴了還是便宜。匯出圖片同樣帶出充電資訊與走勢圖。"
        ]),
        ChangelogEntry(version: "25.327", build: 1080, date: "2026/08/19", notes: [
            "【新增】統計圖表匯出前多了「顯示名字」選擇：可選「全部顯示」，或只顯示某一個人的名字——其他人的長條保留（看得出相對位置與團隊分佈），名字改以「同仁1、同仁2…」匿名，團隊平均照常顯示。單張圖與全部圖表的匯出都適用。",
            "【說明】匿名編號依成員順序固定：同一次匯出的每張圖裡，「同仁1」都是同一個人，跨圖對照不會亂。適合把個人成績單分享給本人看，不揭露其他同仁的名字。"
        ]),
        ChangelogEntry(version: "25.326", build: 1079, date: "2026/08/19", notes: [
            "【調整】統計圖表的「任務完成數」不再計入「應做未作為（缺失）」的任務——缺失結案是扣分事項，不是產出，混進完成數會把缺失多的人墊高成高產出。圖表下方說明同步更新。"
        ]),
        ChangelogEntry(version: "25.325", build: 1078, date: "2026/08/19", notes: [
            "【新增】任務可標記為「應做未作為（缺失）」：任務編輯的「任務資訊」多了紅色開關——打開後任務轉為扣分事項，任務分數自動設為 -1（仍可用加減微調）；缺失任務的分數固定存自訂值、不跟隨全域權重。",
            "【新增】缺失任務全面紅色樣式：任務卡片抬頭改紅色警示三角＋「性質：應做未作為（缺失）」欄位；部屬頁任務列表與行事曆搜尋的任務膠囊顯示紅色「缺失」徽章。關掉開關即回復一般任務（分數回到預設）。"
        ]),
        ChangelogEntry(version: "25.324", build: 1077, date: "2026/08/19", notes: [
            "【新增】會議卡片可以就地「加開一場」了：場次／議程清單正下方多了「加開一場」按鈕，點開只填日期時間、按加開即完成——不用再進編輯頁面繞一圈。加開的臨時場次跟編輯頁加開的是同一種東西，之後在卡片上點該場次即可填議程、改期或取消。"
        ]),
        ChangelogEntry(version: "25.323", build: 1076, date: "2026/08/19", notes: [
            "【新增】公司組織人員卡片多了「相關作品・報告・改善」區塊：是你部屬的人，自動帶入他的報告（含分類徽章與完成勾）與改善／成就紀錄（各取最新 8 筆）；名片連結下方就看得到他做過什麼。",
            "【新增】非部屬的其他部門同仁也能記報告了：卡片按「新增項目」，填項目名稱＋日期即可（簡易紀錄），長按項目可刪除。",
            "【新增】公司組織頁面頂部加搜尋列：可搜人員（姓名／職稱／部門）、報告（部屬報告＋簡易項目）與改善項目（改善／成就紀錄），結果分三區、點任一列直接開該人員的卡片。"
        ]),
        ChangelogEntry(version: "25.322", build: 1075, date: "2026/08/19", notes: [
            "【新增】統計圖表補上「總分（目前）」排行：主動性與潛力的平均，與部屬列表左側的綜合分數同一套算法（金色獎盃圖示）。",
            "【新增】每張統計圖都插入灰色的「團隊平均」對照條：對全部成員取平均（含數值為 0 的人，才是真平均），依大小自然排進條列裡——誰在平均之上、誰在平均之下一眼看出。分享匯出的圖同樣帶平均條。"
        ]),
        ChangelogEntry(version: "25.321", build: 1074, date: "2026/08/19", notes: [
            "【修正】人才矩陣切到「統計圖表」分頁後，右上分享按鈕仍匯出矩陣圖的問題：分享現在跟著分頁走——矩陣分頁維持原本的整張矩陣匯出；統計分頁按分享會跳選單，可選「全部圖表（一張圖）」或任一單張圖表。",
            "【新增】每張統計圖卡右上多了小分享鈕：一鍵把該張圖（含標題、說明與匯出戳記）匯出成圖片分享，不用經過選單。"
        ]),
        ChangelogEntry(version: "25.320", build: 1073, date: "2026/08/19", notes: [
            "【新增】人才矩陣分成兩個分頁（比照法人連續買超）：「人才矩陣」維持既有散布圖；新的「統計圖表」以人為單位做橫條圖統計，年度可用膠囊切換（資料出現過的年份），部門篩選兩個分頁共用。",
            "【新增】年度統計四張：請假時數（含不計分假別）、任務完成數、報告完成數、加分紀錄（優點＋成就＋進步）與扣分紀錄（缺點＋缺失＋疏失）；即時狀態三張：逾期件數（與逾期扣分同定義）、主動性分數與潛力分數排行（與矩陣同一套計算，含被標註加分）。",
            "【說明】每張圖依數值大到小排序、條尾標數值、零值的人不佔版面；全部為零顯示「此範圍沒有資料」。"
        ]),
        ChangelogEntry(version: "25.319", build: 1072, date: "2026/08/19", notes: [
            "【改版】重大決議的參照前案改為「章節式」呈現（使用者指定）：不再把前案文字插進你的內容欄位，改成獨立的「參照前案」章節——每個前案一小節，完整顯示標題、流水號、日期與內文，插在「決議內容」章節上方；標題列可點直接跳到前案卡片。",
            "【調整】編輯畫面的參照前案區同步顯示前案內文（唯讀灰底），內容欄位保持乾淨只寫本次的事；匯出圖片與文字的章節順序一致（參照在上、決議內容在下，文字匯出附前案內文）。先前已被插進內容的引用文字不會自動移除，可自行刪掉。"
        ]),
        ChangelogEntry(version: "25.318", build: 1071, date: "2026/08/19", notes: [
            "【修正】兼任待辦的系統分類：輸入自訂分類（例：尾牙）沒按 + 直接按儲存，文字會默默遺失——現在儲存時視同已確認、直接加入分類（重大決議的分類輸入也一併修正，行為與報告分類一致）。",
            "【修正】兼任待辦的截止日存不進去：打開「設定截止日」後若沒動過日期選擇器（想用畫面顯示的今天），按儲存截止日其實是空的——選擇器顯示的只是畫面預設值。現在開關一打開就把預設日期寫進資料，儲存時再做一層保險。"
        ]),
        ChangelogEntry(version: "25.317", build: 1070, date: "2026/08/19", notes: [
            "【改版】財富卡片的柱狀圖不再分頁：銀行/證券卡片的餘額柱狀圖與信用卡的消費趨勢圖，原本每 30 筆切一頁要左右滑，改用與趨勢曲線同一套壓縮模板——第一根與最後一根保留真實值、中間依區段平均壓到固定 30 根，一張圖看完整段歷史，每一筆資料都貢獻到圖上而不是被丟掉。",
            "【說明】資料超過 30 筆時圖下方會標示「已壓縮顯示（共 N 筆）」；30 筆以內維持原樣逐筆顯示。壓縮演算法收進趨勢模板（HeroTrendSeries）供全 App 共用。"
        ]),
        ChangelogEntry(version: "25.316", build: 1069, date: "2026/08/19", notes: [
            "【修正】買美股的扣款金額少了匯率換算（使用者回報）：美股交易金額是美元，寫入財富卡片（銀行/證券帳戶）的扣款卻直接把美元數字當台幣記——買 1,000 美元只扣 NT$1,000。現在台幣帳戶會按 USD→TWD 匯率換算成台幣扣款；外幣（USD）帳戶維持原幣金額不動。",
            "【修正】美股配息同步一併校正：入帳到台幣帳戶按匯率換算；「配息收入」帳本一律以台幣統計。台股完全不受影響。既有寫錯的扣款紀錄，重新開該股的交易編輯按一次儲存就會整批重寫成正確金額。"
        ]),
        ChangelogEntry(version: "25.315", build: 1068, date: "2026/08/19", notes: [
            "【新增】警報的處理過程回寫時間軸：機台警報自動掛任務給負責人後，時間軸的警報列下方會出現「處理回報」子項目——處理中橘色、已完成回報綠色，並帶處理人姓名；點一下展開查看完整處理過程（處理措施、回復結果、完成時間）。",
            "【說明】機台詳情卡與部屬執掌頁的兩條時間軸都有；資料直接讀任務的回報欄位（任務編輯裡填的處理措施／回復結果），任務更新時間軸即時跟著變，不另存副本。手動補登的舊警報沒有對應任務就不顯示子項目。"
        ]),
        ChangelogEntry(version: "25.314", build: 1067, date: "2026/08/19", notes: [
            "【修正】儲蓄險總期數少算一期的問題（使用者回報）：繳款發生在每期期初（生效日當天就扣第一期），保單到期日又常寫成「差一天滿整數年」（例：2018/08/11 生效、2028/08/10 到期），原本用整除月數計算會把 10 期算成 9 期。現在未整除的餘月正確進位——那代表最後一期已在期初扣款、只是該期還沒走完。恰好整年（8/11 到 8/11）的保單不受影響。",
            "【說明】總期數修正後，到期預估領回、已繳總額、進度條與甘特圖等以期數推算的數字都會同步校正，不需要重新輸入保單。"
        ]),
        ChangelogEntry(version: "25.313", build: 1066, date: "2026/08/19", notes: [
            "【新增】充電支出多了「里程錶」欄位（選填）：記下充電當下的里程錶讀數，連續兩筆都有填時自動算出該區間的電耗——車輛卡片的充電統計多一列 KPI（平均電耗 km/kWh、每公里電費、統計里程）與「電耗走勢」圖（越高越省電）。",
            "【說明】電耗採充電樁法：行駛距離＝兩次充電的里程差、耗能≈後一次充入度數；里程差為負（誤填）或超過 2,000km（中間漏記太多次）的區間自動剔除不入統計。",
            "【修正】修正 25.312 的一處編譯錯誤（充電欄位狀態宣告漏字）。"
        ]),
        ChangelogEntry(version: "25.312", build: 1065, date: "2026/08/19", notes: [
            "【新增】電動車充電支出可以記度數與電量了：新增汽車「電費」變動支出時，多了「充電度數（kWh）」與「電量 從 % → 到 %」欄位（皆選填），輸入度數與金額會即時試算這次的每度電價。",
            "【新增】車輛卡片多了「充電統計」區塊：四格 KPI（平均每度電價、平均每次度數、累計度數、推估電池容量）＋兩張趨勢圖——「每度電價走勢」看充電成本變化、「推估電池容量走勢」（度數 ÷ 充電區間 %）當電池健康度的近似觀察，容量曲線緩緩下滑＝正常老化。",
            "【說明】推估容量只採計充電區間 ≥15% 的紀錄（區間太小誤差會放大）；沒有任何度數紀錄時充電統計區塊不顯示。"
        ]),
        ChangelogEntry(version: "25.311", build: 1064, date: "2026/08/19", notes: [
            "【新增】照片區的 ＋ 選單多了「掃描文件（自動裁切）」：拍帳單、保養單、文件時走系統掃描器——自動偵測文件邊框、拍完自動透視校正拉成端正的長方形（跟名片掃描同一套），歪著拍也不怕。可一次連掃多頁，逐頁存成照片。",
            "【說明】所有用共用照片廊的地方一次到位：收支帳單照片、水電收據、電梯保養、裝潢照片等。一般「拍照」與「相簿多選」照舊保留，拍現場實景用拍照、拍紙本文件用掃描。"
        ]),
        ChangelogEntry(version: "25.310", build: 1063, date: "2026/08/19", notes: [
            "【新增】進階設定多了「法人快照儲存」：法人買賣超每日快照的保留天數開放調整（5～365 天，預設 30）——天數越多，訊號追蹤能回測的區間越長。調小會立刻刪除最舊的快照。",
            "【新增】容量統計與容量反推：頁面顯示已存快照份數、佔用容量與平均每日大小；也可以反過來輸入「容量上限（MB）」，用平均每日大小反推可存天數、一鍵套用。",
            "【調整】法人連續買超頁的天數上限與說明文字跟著保留天數設定連動，不再寫死 30 天。"
        ]),
        ChangelogEntry(version: "25.309", build: 1062, date: "2026/08/19", notes: [
            "【新增】法人連續買超頁分成兩個分頁：「連買統計」維持既有的即時篩選；新的「訊號追蹤」找出歷史上「連買達 N 天」的完成日（例：8/15 完成連買 5 天），顯示完成日收盤到 M 個交易日後的漲跌幅——連買門檻與追蹤天數都可用滑桿調整（例：追 10 天或 20 天）。",
            "【新增】訊號追蹤頂部有回測統計：已滿追蹤天數的訊號共幾筆、上漲比例、平均漲跌幅（漲紅跌綠），一眼看出這個訊號到底準不準。還沒滿追蹤天數的訊號先用最新收盤計算並標「追蹤中（第 X 天）」。",
            "【說明】訊號依日期新到舊排列、以量能取前 40 筆（要逐檔抓股價，限量控制流量）；股價逐檔載入時清單會即時補上漲跌幅。"
        ]),
        ChangelogEntry(version: "25.308", build: 1061, date: "2026/08/19", notes: [
            "【改版】電梯保養記錄的照片改用全 App 共用的照片廊模組（與帳單照片、裝潢照片、水電收據同一套）：可拍照連拍、相簿一次多選、縮圖橫列、點開燈箱檢視、逐張刪除——不再是只能放一張的陽春介面。",
            "【改版】保養照片升級為可多張（保養單、銘板、現場照一次收齊）：舊資料的單張照片自動升級進新格式，列表上的照片鈕會顯示張數，房屋資料照片牆也會帶出全部照片。"
        ]),
        ...
        ChangelogEntry(version: "25.305", build: 1058, date: "2026/08/19", notes: [
            "【修正】被擁有者移出家人共享後，被移出那台卡死在共享模式的問題（使用者回報）：退出鈕按了顯示「退出失敗」、讀寫全掛、也無法重新加入。原因是退出只認一種伺服器錯誤碼，被移出後伺服器回的是權限錯誤，退出永遠失敗、本機一直留在參與者模式。現在「其實已經不在共享裡」的各種錯誤（zone 不見、權限失敗、項目不明等）一律視同退出成功，直接切回自己的資料庫。",
            "【新增】被移出／擁有者停止共享時自動偵測：下一輪同步收到 zone 不見或權限錯誤，這台會自動退回私有模式——退出鈕消失、資料恢復讀寫（用自己的雲端），之後點新的邀請連結即可重新加入。",
            "【新增】退出真的失敗時（例如沒網路）多了「強制退出（僅本機）」按鈕：不等伺服器、直接切回自己的資料庫，資料保留並推回自己的雲端。"
        ]),
        ChangelogEntry(version: "25.304", build: 1057, date: "2026/08/19", notes: [
            "【改版】股票明細的圖片匯出改接共用分頁模板：先前只匯出閃卡＋K線就結束，現在完整帶出閃卡、K線、交易紀錄逐筆（買賣徽章、張數×單價、金額）與股利紀錄逐筆（配息／配股、每股金額、總額）。",
            "【新增】匯出前跳「每頁項目數」選單（與其他頁面同一套）：可輸入數字分頁、也可「全部一頁」；每一頁重複「📈 股票｜名稱（代號）」抬頭，跨頁切在紀錄中段時下一頁自動補區塊標題，頁尾帶頁碼與匯出時間。沒有任何交易／配息的股票維持原本單張閃卡＋K線匯出、不多問。"
        ]),
        ChangelogEntry(version: "25.303", build: 1056, date: "2026/08/19", notes: [
            "【新增】機台詳情的「PM／警報時間軸」右上多了＋按鈕：不用進編輯畫面，直接快速新增「PM 停機」「PM 完成復機」或「警報」——填時間與內容一鍵寫入；快速新增的警報照舊自動掛任務給機台負責人。",
            "【新增】PM 記錄可標註階段：停機（橘色、暫停圖示）／完成復機（綠色、勾勾圖示）——停機一筆、復機一筆，時間軸直觀切分保養停機的區間；有標階段的 PM 連時間一起顯示，同一天停機→復機順序分得出來。編輯表單的每筆 PM 也多了「一般／停機／復機」切換。",
            "【調整】機台詳情與部屬執掌頁的時間軸圖例更新為：停機（橘）／PM・復機（綠）／警報（紅）。舊有未標階段的 PM 照常顯示為綠色「PM 保養」。"
        ]),
        ChangelogEntry(version: "25.302", build: 1055, date: "2026/08/19", notes: [
            "【改版】逾期扣分規則（使用者指定）：任務一旦逾期，扣分就定案——逾期後才補完成的任務照樣扣逾期分，完成加分與逾期扣分互抵（預設 +3／-3 等於 0 分），不再因為補打勾就拿回逾期扣分。逾期判定：未完成看截止日是否已過；已完成看完成時間是否晚於截止日（報告以報告日為準）。",
            "【調整】評分明細的逾期列改為「逾期（含補完成）×N」；進階設定的逾期 Offset 說明同步更新。舊資料沒記完成時間的已完成項目從寬不列入逾期。"
        ]),
        ChangelogEntry(version: "25.301", build: 1054, date: "2026/08/19", notes: [
            "【新增】固定支出的金額走勢圖可以互動了：手指按住在圖上左右滑，會吸附到最近的資料點並顯示細項小卡——日期、金額（NT$ 大字）、與前一筆的增減金額與百分比（漲紅跌綠），預測點會標示「預測」膠囊。放開手指小卡消失。匯出圖片維持乾淨的靜態版。"
        ]),
        ChangelogEntry(version: "25.300", build: 1053, date: "2026/08/19", notes: [
            "【新增】固定支出卡片可「更新金額」了：像乙式險每年續保金額都變，按卡片「金額走勢」區的更新按鈕輸入新金額——目前金額改為新值，舊金額自動保留成歷史點（首次更新會把原金額連同起始日期補成起點）。",
            "【新增】備註上方的「金額走勢」曲線：實線畫出歷年金額變化（平滑曲線＋節點），虛線以線性回歸往後預測 3 年（每年一點、不會低於 0），附實線／虛線圖例。累積兩筆金額就會出現曲線。",
            "【新增】匯出圖片會帶上走勢圖（有兩筆以上才畫）；匯出文字附「📈 金額走勢」逐年清單。編輯表單存檔不會弄丟金額歷史。"
        ]),
        ChangelogEntry(version: "25.299", build: 1052, date: "2026/08/19", notes: [
            "【新增】重大決議有流水號了（#001、#002…）：同一職務內自動遞增編號，列表、詳情卡片與文字匯出都會顯示；既有決議依日期由舊到新自動補號。",
            "【新增】決議可「參照前案」（可多選）：編輯畫面多了參照前案區，從本職務的其他決議勾選（可搜尋標題/內容/流水號）。勾選時會自動把前案的決議日期＋標題＋內容以引用區塊帶進內容欄位，接著往下寫本次的內容即可。",
            "【新增】決議卡片底部出現「參照前案」超連結列：點一下直接跳到該前案的決議卡片（可一層層追溯）；匯出圖片與文字也會列出參照清單。前案被刪除會顯示提示、不會壞掉。"
        ]),
        ChangelogEntry(version: "25.298", build: 1051, date: "2026/08/19", notes: [
            "【改版】部屬頁面看板下方的部門篩選：從橫向膠囊列改為下拉選單，且支援多選——勾選期間選單不會收合，一次勾好幾個部門；「全部部門」一鍵清空。按鈕上顯示目前的選擇摘要（最多列 2 個部門名，其餘以「等 N 個部門」帶過）與已選數量。",
            "【調整】部屬清單標題與人數統計跟著多選結果連動；勾選中的部門被刪除時自動從篩選剔除。原本的單選設定會自動沿用，不用重設。"
        ]),
        ChangelogEntry(version: "25.297", build: 1050, date: "2026/08/19", notes: [
            "【修正】編輯設備選了所屬部門（如：實驗室），負責人選單帶出的卻不是該部門設定的人員——原因是部門同步只有「部屬→組織人員」單向，從公司組織頁把人調到某部門時，部屬端還停在舊部門，設備編輯器又只認部屬端的部門。現在改成雙向：組織人員調部門會連動更新部屬的部門；負責人判定也同時認組織頁設定的部門，舊資料不用重存就對得上。",
            "【改版】負責人選單依所選部門分組：「〇〇 成員」一組在前、「其他部屬」一組在後，一眼看出誰是這個部門的人，跨部門支援時也還是選得到其他人。"
        ]),
        ChangelogEntry(version: "25.296", build: 1049, date: "2026/08/19", notes: [
            "【修正】部屬「執掌」分頁的執掌設備清單：系統別／部門名稱字多時，膠囊被擠在同一行互相壓縮變形——改成自動換行排列，放不下就換到下一行，每顆膠囊維持完整形狀與完整文字。",
            "【修正】公司組織部門詳細頁的「部門所屬設備」清單同步修正：膠囊（PM／警報／系統別／負責人）一樣改為自動換行，點系統別或負責人膠囊篩選的功能不變。"
        ]),
        ChangelogEntry(version: "25.295", build: 1048, date: "2026/08/19", notes: [
            "【新增】部屬總覽看板多了生日蛋糕按鈕：今天或明天有人生日，蛋糕會亮起來（粉紅色＋光暈）；沒有的話蛋糕維持黑白。名單涵蓋部屬與公司組織人員（同一個人不重複列）。",
            "【新增】點蛋糕彈出「生日名單」視窗：分「今天生日」「明天生日」與「即將到來（30 天內）」三區，顯示每個人的姓名、身分（部屬職稱／組織人員部門）、星座與生日日期，即將到來的還會標示剩幾天。"
        ]),
        ChangelogEntry(version: "25.294", build: 1047, date: "2026/08/19", notes: [
            "【新增】任務可以有自己的分數：新增／編輯任務的「任務資訊」右側多了加減按鈕，可為單一任務設定完成時計入主動性評分的分數（可正可負）。沒有調整就跟隨進階設定的「完成任務」權重（顯示「預設」標籤），之後在進階設定調整權重，未自訂的任務會跟著變。",
            "【新增】有自訂分數的任務，在任務列表會顯示分數膠囊（正分綠色、負分紅色）；評分明細裡自訂分任務獨立成「完成任務（自訂分）」一列，與預設分任務分開檢視。"
        ]),
        ChangelogEntry(version: "25.293", build: 1046, date: "2026/08/19", notes: [
            "【新增】履歷里程碑（入職／升職／調轉）打開「是否為管理職」後，下方長出「管理單位」欄位（單選）：可以自己輸入，也可以點下方公司組織的部門膠囊快速帶入。職涯列表的管理職徽章會顯示成「管理職・製造一課」。",
            "【新增】管理單位名稱與公司組織的部門相符時，該部門詳細頁的「管理人員」名單會自動出現「我」（帶橘色標記，顯示個人資料的姓名）——依履歷推導自動帶入，不用手動把自己建成組織人員。已離職或最新職位不是管理職就不顯示。",
            "【修正】編輯兼任職務的里程碑按儲存，會把累積的「重大決議」整批清空的問題（重建紀錄時漏帶這個欄位，待辦／成員／會議／重要日期都有帶、唯獨決議漏了）。"
        ]),
        ChangelogEntry(version: "25.292", build: 1045, date: "2026/08/19", notes: [
            "【新增】公司組織人員的卡片也能設生日提醒了：卡片上的生日膠囊補上星座，右邊多了鈴鐺（已建立提醒亮黃色），點開與部屬相同的提醒設定頁——提前 N 天、連續 N 年或無限期、可選寫入哪個行事曆。刪除人員或把生日欄位拿掉時，行事曆上的提醒會自動移除。",
            "【改版】生日提醒的行事曆事件內容全面完整化：標題變成「🎂 姓名 生日｜部門 職稱」一眼認得出是誰；事件內容列出身分（部屬／組織人員）、部門、職稱、到職日、出生日期與星座、提醒設定摘要（提前幾天、重複到哪一年或無限期）。既有提醒按一次「更新提醒」即可換成新格式。",
            "【修正】編輯組織人員按儲存會弄丟生日提醒連結（鈴鐺變回白色）的潛在問題。"
        ]),
        ChangelogEntry(version: "25.291", build: 1044, date: "2026/08/19", notes: [
            "【修正】報告編輯的分類：輸入自訂分類（例：TB）按 + 後，膠囊列會立刻出現該分類並呈選取狀態——先前按 + 只清空輸入框、畫面毫無變化，看起來像文字被吃掉。",
            "【修正】自訂分類輸入框裡還沒按 + 的文字，按右上角儲存時會直接視同已確認的分類存進報告，不再默默遺失。"
        ]),
        ChangelogEntry(version: "25.290", build: 1043, date: "2026/08/19", notes: [
            "【改版】我的行事曆的「部屬任務」區塊改為「即將逾期任務」：只列出截止日壓在當日的未完成任務——今天非處理不可的緊急項目。任務日期落在當日的一般任務，部屬總覽的「當日任務」已涵蓋，不再重複顯示。"
        ]),
        ChangelogEntry(version: "25.289", build: 1042, date: "2026/08/19", notes: [
            "【修正】照片大圖檢視的貼合規則改為明確計算：比較「寬度貼齊」與「高度貼齊」兩種縮放，取不會出血的那一種——寬度貼齊會讓高度爆出畫面時，自動改用高度貼齊，保證整張照片打開就完整在畫面內、兩個方向都不出血。縮放、平移、雙擊行為不變。"
        ]),
        ChangelogEntry(version: "25.288", build: 1041, date: "2026/08/19", notes: [
            "【新增】收支卡片的分享選單多了「只匯出帳單照片（N 張）」——只分享上傳的照片原檔、不帶卡片圖（變動支出與固定支出）。",
            "【修正】編輯支出時選了外幣，最上方的金額預覽看板仍硬顯示 NT$ 的問題——現在會以所選幣別顯示原幣金額，有匯率時附 NT$ 等值換算（例：USD 100 ≈ NT$3,200）。",
            "【修正】帳單照片點開的大圖檢視：打開就是完整貼合畫面（最長邊貼齊、不出血），支援雙指放大縮小（縮小於貼合尺寸會回彈）、放大後可拖曳平移、雙擊在貼合與 2.5 倍之間切換。"
        ]),
        ChangelogEntry(version: "25.287", build: 1040, date: "2026/08/19", notes: [
            "【新增】收支卡片匯出圖片時，上傳過的帳單照片原檔會一併附進分享——一次分享＝卡片圖＋全部照片（原始解析度），選單會標示「含帳單照片 N 張」。變動支出與固定支出都適用。"
        ]),
        ChangelogEntry(version: "25.286", build: 1039, date: "2026/08/19", notes: [
            "【新增】收入與變動支出的項目點開先看詳情卡片（共用同一個卡片模組）：類別圖示＋金額大字、類別/日期/週期、同行成員/社交對象/店家地址、入帳銀行/扣款目標，備註可就地編輯（點鉛筆），變動支出含帳單照片區；右上「編輯」才進編輯表單。固定支出維持原本的專屬卡片（儲蓄險/貸款甘特圖）。",
            "【新增】三種卡片都可以匯出分享了：右上分享鈕可匯出圖片（乾淨的分享版、含匯出時間戳記、檔名帶項目名稱）或 Emoji 排版純文字（LINE／訊息直接貼）；固定支出的匯出會帶出儲蓄險與貸款明細。"
        ]),
        ChangelogEntry(version: "25.285", build: 1038, date: "2026/08/19", notes: [
            "【調整】匯出前的「每頁項目數」從預設清單改為兩個選項：直接輸入數字（預填上次輸入的值）、或「全部一頁」——想要幾個就打幾個，不必在固定選項裡挑。所有分頁匯出（行事曆搜尋、部屬總覽、家庭、兼任職務工作區）一併生效。"
        ]),
        ChangelogEntry(version: "25.284", build: 1037, date: "2026/08/19", notes: [
            "【改版】分頁產圖模組全面上線：部屬總覽（請假／報告／會議／當日任務／未完成會議條目／未完成任務）、家庭頁（待辦／成員／全部）、兼任職務工作區（五區塊）的圖片匯出全部換到 25.283 的分頁模組——每一頁都重複抬頭與區塊標題（跨頁切在區塊中段時下一頁自動補標題），切點永遠落在項目之間，頁尾帶頁碼與匯出時間。",
            "【新增】以上匯出按下去都會先跳「每頁項目數」選單（10／15／20／30／全部一頁，顯示會產生幾頁，記住上次選擇）。總覽看板（單卡）與空區塊維持整張匯出不多問。"
        ]),
        ChangelogEntry(version: "25.283", build: 1036, date: "2026/08/19", notes: [
            "【新增】分頁產圖模組（第一站：行事曆搜尋結果匯出）：不再用長圖硬切——改以「項目」為單位分頁，每一頁都重複抬頭（搜尋詞＋筆數），第二頁起不再沒頭沒尾；頁尾帶「第 N 頁／共 M 頁・匯出時間」，切點永遠落在項目之間不會切到一半。",
            "【新增】匯出前先選每頁項目數：按匯出會先跳選單（10／15／20／30 或全部一頁，並顯示會產生幾頁），上次的選擇記住排最前面。部屬總覽、家庭、兼任職務的匯出將陸續換到同一套模組。"
        ]),
        ChangelogEntry(version: "25.282", build: 1035, date: "2026/08/19", notes: [
            "【改版】週五套大夜輪班改為自動判斷（不用再自己選）：點週五時看同廠區的週六晚上有沒有其他人輪大夜——有人（代表對方輪班到週日結尾、週日交接重疊）就套「五時差、六休、日起大夜到下週五」；週六沒人就套「五時差、六起大夜到下週四、下週五休」。按鈕文案會直接顯示判斷結果（該廠週六已有人／無人），按下去就是對的序列。"
        ]),
        ChangelogEntry(version: "25.281", build: 1034, date: "2026/08/19", notes: [
            "【改版】大夜班輪班邏輯逐日確認版：點哪天套用，就是「那天時差、隔天起大夜 6 晚、第 8 天休」——按週一＝一時差、二～日大夜、下週一休；按週二＝二時差、三～下週一大夜、下週二休…以此類推（25.280 的對齊週日版本作廢）。",
            "【新增】週五是分岔點，按週五時提供兩種輪法讓你選：①五時差、六起大夜、下週五休（標準 8 天）②五時差、六休（本來就是假日）、週日起大夜 6 晚。交接落在週日時新舊兩人重疊一天，是「每天每廠都要有人」覆蓋規則的正常現象，各自套各自的序列即可。"
        ]),
        ChangelogEntry(version: "25.280", build: 1033, date: "2026/08/19", notes: [
            "【改版】大夜班輪班邏輯照實際排班規則調整：套用大夜輪班時，對齊該週填「週日～週四大夜、週五＋週六時差假」——大夜是週日晚上上到週五早上下班，時差本來落在週六，但週六是假日等於沒補到、要補一天還他，所以週五也標時差。不論點到該週哪一天，套用的都是同一週的這個型（原本是從點選日起跑 8 天的通用序列）。",
            "【新增】班表格子的快速按鈕多了「設為假日值班（單日）」，排在設定/變更班別下面——假日值班不必再進下拉選單裡找。"
        ]),
        ChangelogEntry(version: "25.279", build: 1032, date: "2026/08/19", notes: [
            "【調整】會議詳情的產生時間移到最下面，字樣改為「會議產生」——詳情卡片、匯出圖片與匯出文字都一致押在底部。",
            "【修正】匯出圖片不再出現編輯用的元素：備註等欄位是空的就整塊不顯示，有內容也不會帶鉛筆按鈕與佔位提示——匯出的是乾淨的分享版。",
            "【調整】介面上空欄位的佔位文字統一簡化為「（未填）」（原本的「未填，點筆直接補」太長）；警報任務的處理措施／回復結果簡化為「（尚未回報）」。"
        ]),
        ChangelogEntry(version: "25.278", build: 1031, date: "2026/08/19", notes: [
            "【修正】部分同仁設定生日提醒後鈴鐺不變黃的問題。原因：行事曆事件建立成功、但某些行事曆來源（iCloud 同步中／Google／Exchange）當下拿不到事件識別碼，App 以為建立失敗、沒記下對應——行事曆裡有事件、鈴鐺卻不亮。現在改用一定拿得到的備援識別碼，查找與刪除兩種識別碼都認得；建立成功後並會自動清掉同一人先前沒對應到的舊提醒事件（不會留重複的）。鈴鐺沒亮的同仁：再按一次「建立提醒」即可修復並亮起。"
        ]),
        ChangelogEntry(version: "25.277", build: 1030, date: "2026/08/19", notes: [
            "【新增】生日提醒可選「無限期」：打開開關就不設迄日，每年提醒直到手動移除；關閉則照舊輸入連續幾年。",
            "【修正】「設 5 年但行事曆只看得到兩年」的問題：循環規則從「次數（COUNT）」改為「明確迄日（UNTIL）」——部分行事曆來源（Google／Exchange）對次數規則的展開顯示不一致，迄日在所有來源行為一致。另外更新提醒改為「先建立新的、成功後才刪除舊的」，反覆調整設定不會殘留舊佔位，失敗時原提醒也不會消失。已建立的提醒請再按一次「更新提醒」套用新規則。"
        ]),
        ChangelogEntry(version: "25.276", build: 1029, date: "2026/08/19", notes: [
            "【新增】生日提醒可以選擇寫入哪個行事曆了（比照新增會議）：設定頁多了「寫入行事曆」選單，列出所有可寫入的行事曆（色點＋名稱）。預填順序：既有提醒所在的行事曆 → 上次選過的 → 系統預設；換行事曆再按更新，事件會搬到新的行事曆。"
        ]),
        ChangelogEntry(version: "25.275", build: 1028, date: "2026/08/19", notes: [
            "【新增】部屬生日：編輯部屬多了「填入生日」欄位（選日期即時顯示星座）；部屬明細看板在入職日期右邊顯示生日徽章（🎂 月/日・星座），最右邊有鈴鐺按鈕。",
            "【新增】生日 Apple 行事曆提醒：點鈴鐺設定「提前幾天提醒」（數字輸入）與「連續提醒幾年」，寫入行事曆為每年重複的全日事件、提前 N 天上午 9:00 跳提醒；已建立過會更新同一個事件不重複建立（鈴鐺亮黃色表示已設定），可一鍵移除，刪除部屬時提醒一併清除。",
            "【修正】編輯部屬基本資料會把「升職歷程」清空的潛在問題（重建時漏帶欄位，本次補上）。"
        ]),
        ChangelogEntry(version: "25.274", build: 1027, date: "2026/08/19", notes: [
            "【新增】評分權重開放調整：設定 → 進階設定 → 評分權重。主動性（基礎分、完成任務/議程/報告、被標註、兼任待辦、請假每 8 小時）與潛力（基礎分、成就/優點/進步、缺點/缺失、疏失三級）的加減分全部可用步進器調整，一鍵恢復預設值；調整後所有畫面立即以新權重重算，既有紀錄不需重新輸入。",
            "【新增】逾期 Offset：可設定「每項逾期未完成固定扣 X 分」——過截止日的任務與過報告日的報告都算一項，逾期對分數的影響一目了然，不用自己換算。預設 0（不扣，維持原本行為）；設定後評分明細會多一列「逾期未完成 ×N」。"
        ]),
        ChangelogEntry(version: "25.273", build: 1026, date: "2026/08/19", notes: [
            "【新增】浮動「新增收支」按鈕可以關掉了：設定 → 進階設定 → 介面，新增「浮動新增收支按鈕」開關（預設顯示）。有使用者反映用不到這顆按鈕——關閉後首頁右下角不再顯示，仍可從收入頁與各總覽頁的 ＋ 新增收支；隨時可再打開。"
        ]),
        ChangelogEntry(version: "25.272", build: 1025, date: "2026/08/19", notes: [
            "【新增】兼任職務工作區的五個區塊（待辦、重要日期、成員名單、會議紀錄、重大決議）可收合：區塊標題右側多了收合箭頭，收起後只留標題列與計數；收合狀態會記住——這次收起來，下次打開還是收著。預設全部展開。",
            "【新增】展開的區塊項目超過 10 個時，先列前 10 個，底部出現「顯示其他 N 項」按鈕，每按一次再多顯示 10 個——上百筆決議不再一次撐爆頁面。搜尋或篩選進行中會自動強制展開並顯示全部命中項目（不然搜到的會被收合藏住）；匯出圖片一律完整輸出、不受顯示窗影響。"
        ]),
        ChangelogEntry(version: "25.271", build: 1024, date: "2026/08/19", notes: [
            "【新增】金額欄位內建計算機（全 App 模組）：記帳輸入金額旁多了一顆計算機按鈕，點開半頁計算機做基本加減乘除（先乘除後加減、顯示算式與結果預覽），按「帶入金額」把結果直接填進欄位——多筆發票加總、帳單分攤不必再切到計算機 App。變動/固定支出的金額與總貸款金額、收入金額都已裝上；開計算機時會自動帶入欄位既有金額當起始值，除以 0 或算式不完整時帶入鈕反灰。"
        ]),
        ChangelogEntry(version: "25.270", build: 1023, date: "2026/08/19", notes: [
            "【新增】重大決議詳情卡片：點決議列改為先開檢視卡片（不再直接進編輯），為分享匯出設計——標題全文＋所屬職務、決議日期、廠區、系統分類（彩色膠囊）、發起人、決議內容全文一次帶齊。右上分享鈕可匯出圖片（乾淨的分享版卡片、含匯出時間戳記、檔名帶決議標題）或 Emoji 排版純文字（LINE／訊息直接貼）；「編輯」才進編輯表單。",
            "【新增】卡片上的決議內容支援就地編輯（點鉛筆直接改，比照 25.266 模板）；行事曆搜尋點開決議也改為先顯示卡片。"
        ]),
        ...
        ChangelogEntry(version: "25.267", build: 1020, date: "2026/08/19", notes: [
            "【改版】重大決議與兼任待辦的「系統分類」從固定選單（CDA/BGS/…）改為開放式：自由輸入你自己的專業分類（不限氣體化學領域），key 過的分類自動變成膠囊、下次點一下帶入——與廠區、決議發起人同一套記憶模式，重大決議與待辦共用同一份歷史、全部兼任職務共享。",
            "【說明】原本用固定選單存的分類完全保留（自動視為文字分類，膠囊照常出現、篩選與搜尋照常）；分類膠囊顏色改為依名稱固定配色，同一分類在任何裝置永遠同色。工作區點膠囊篩選、行事曆搜尋、圖片與 CSV 匯出全部照舊支援。"
        ]),
        ChangelogEntry(version: "25.266", build: 1019, date: "2026/08/19", notes: [
            "【新增】詳情卡片就地編輯（全 App 模板）：任務／會議／報告／請假／紀錄詳情卡片與機台詳情卡片上的多行文字區塊——備註、內容、事由、處理措施、回復結果——右上都有一顆鉛筆按鈕，點下去原地變成多行輸入框，改完按「儲存」直接寫回、「取消」原樣退回，改一小段字不必再進完整編輯頁。",
            "【調整】這些區塊在內容為空時也會顯示（淡色「未填，點筆直接補」佔位），方便直接補資料；警報任務的處理措施／回復結果未回報時同樣可以在卡片上直接填。任務「內容」就地修改後會同步更新對應的 Apple 提醒事項標題（有開同步時）。"
        ]),
